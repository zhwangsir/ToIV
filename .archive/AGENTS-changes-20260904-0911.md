# AGENTS 变更叙事归档：2026-09-04 ~ 2026-09-11

> 本文件由 2026-09-12 AGENTS.md 精简重构析出，收录原「七、近期关键变更」全量逐条 tip（2026-09-04 ~ 2026-09-11）。
> 更早历史（2026-08-21 ~ 09-03）见 `.archive/AGENTS-full-20260903.md`。
> 活口径（当前焦点 + 未完成任务总表）见仓库根 `AGENTS.md`；机读状态见 `STATE.json`。
...
- **产品 dirty 续（ToIV 开发；via 项目管家）**：dirty（**未** git commit）— **勿 stage**。
  - KSampler `beta57`→`beta`
  - VHS `skip_first_frames` 等
- **复测**：
  - **PASS**：`rh-acc-2218056705`；`rh-acc-3153534978`
  - **仍跟**：`426919` running
  - **LayerMask**：需 worker 上 `models/segformer_b3_clothes` / `segformer_b2_clothes`
  - **Qwen3_VQA**：需 `LLM/Qwen3-VL-4B-Instruct`→`models/prompt_generator/`
- **前置 tip**：`a1db34c`（产品 dirty 回填；144483 PASS）— 已为本地 parent。
- **相关**：STATE `public_e2e_retest_product_dirty_backfill_2026_09_11`；wave4 产品候选 beta57 / LayerMask / Qwen3_VQA。
- **硬口径**：**非** shipped/online；产品仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`live_on_core_dirty_product_beta57_vhs_pass2`（非 shipped）；STATE `public_e2e_retest_product_dirty_beta57_vhs_2026_09_11`；`updated_at` 2026-09-11T10:28:00+08:00。via 项目管家（ToIV 开发）。

### 2026-09-11（项目管家：产品 dirty 回填；144483 PASS）
- **产品 dirty 回填（ToIV 开发；via 项目管家）**：dirty（**未** git commit）— **勿 stage**。
  - easy promptLine `remove_empty_lines`
  - INPAINT `blur_type` / `device_mode`
  - Wan VACE `num_frames`
  - 丢弃无 images 的孤儿 VHS
- **复测**：
  - **PASS**：`rh-acc-1444836353` @ `:8197`
  - `294829`：字段已清，`queued_timeout` @ `:8194` → **重测**
  - `142022`：字段已清 → 缺 `darkBeastMar0326` / `flux2-vae` / `qwen_3_8b` + Flux2 节点（转 **下载/设备**）
- **前置 tip**：`db38be3`（451079 PASS；387721 新缺 SeCVideoSegmentation）— 已为本地 parent。
- **相关**：STATE `public_e2e_retest_wave3_device_2026_09_11`（曾交产品默认回填 easy/INPAINT）；`public_e2e_wave4_2026_09_11`。
- **硬口径**：**非** shipped/online；产品仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`live_on_core_dirty_product_backfill_144483_pass`（非 shipped）；STATE `public_e2e_retest_product_dirty_backfill_2026_09_11`；`updated_at` 2026-09-11T10:26:00+08:00。via 项目管家（ToIV 开发）。
...
- **wave3 设备复测（ToIV 开发；via 项目管家）**：**PASS 1** / **FAIL 5**。
  - **PASS**：`rh-acc-1126448130` @ `:8197`
  - **仍缺设备**：`PDIMAGE:LongerSize`（387721）；`VACE/…_HIGH_bf16` 前缀路径（451079）
  - **交产品默认回填**：`easy promptLine.remove_empty_lines`（294829）；`INPAINT_ExpandMask.blur_type`（142022）
  - **另查**：144483 VHS 缺 images/num_frames；387721 等 PDIMAGE
- **前置 tip**：`7865dd0`（公开 E2E wave3；36 卡 PASS19/FAIL17）— 已为本地 parent。
- **相关**：STATE `public_e2e_wave3_2026_09_11`（wave3 基线 PASS19/FAIL17 分流中设备项本条复测）。
- **硬口径**：**非** shipped/online；产品仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`live_on_core_public_e2e_wave3_device_pass1_fail5`（非 shipped）；STATE `public_e2e_retest_wave3_device_2026_09_11`；`updated_at` 2026-09-11T09:58:00+08:00。via 项目管家（ToIV 开发）。
...
- **214347 PASS；wave2 nodes3 FAIL 均清（ToIV 开发；via 项目管家）**：
  - **PASS**：`rh-acc-2143478786` — 设备 DrawViTPose `crop_y0` 修后 @ `:8197` job=`88a6c2b6…`
  - media：两段 Wanimate audio mp4
  - wave2 nodes3 两 FAIL **均已清**（417975 已在前置 tip PASS；214347 现 PASS）
- **产品**：仍 dirty（已部署、**未** git commit）— **勿 stage**；勿 stage `.regen_tmp`。
- **前置 tip**：`72d158e`（wave2 nodes3 残差；417975 PASS；214347 转设备）— 已为本地 parent。
- **相关**：STATE `public_e2e_retest_wave2_nodes3_fix_2026_09_11`（214347 曾 FAIL→设备；本条清）；`public_e2e_retest_wave2_nodes3_2026_09_11`（PASS1/FAIL2 基线两 FAIL 均清）。
- **硬口径**：**非** shipped/online；产品仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`live_on_core_public_e2e_214347_pass_nodes3_fails_cleared`（非 shipped）；STATE `public_e2e_retest_214347_pass_2026_09_11`；`updated_at` 2026-09-11T08:47:00+08:00。via 项目管家（ToIV 开发）。
...
- **022665 强制 :8196 再 PASS（ToIV 开发；via 项目管家）**：
  - `rh-acc-0226654210` 强制 `:8196` **再 PASS**（设备 QwenVL sm120 sageattn→SDPA 后）
  - job=`f7fbd951…`；log：`Auto mode: Using SDPA` + FP16 override
  - 此前 PASS @ `:8193`（tip `19ce070`）；本条为设备 SDPA 变更后 `:8196` 再核
- **产品**：仍 dirty（已部署、**未** git commit）— **勿 stage**；勿 stage `.regen_tmp`。
- **前置 tip**：`19ce070`（059347/022665 PASS；107403 仍排队）— 已为本地 parent。
- **相关**：前置 STATE `public_e2e_retest_059347_022665_pass_2026_09_11`（022665 曾 PASS @`:8193`）。
- **硬口径**：**非** shipped/online；产品仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`live_on_core_public_e2e_022665_8196_sdpa_pass`（非 shipped）；STATE `public_e2e_retest_022665_8196_sdpa_2026_09_11`；`updated_at` 2026-09-11T07:58:00+08:00。via 项目管家（ToIV 开发）。
...
- **011723 JoyCaption 三机镜像复测（ToIV 开发；via 项目管家）**：
  - `011723` JoyCaption：三机镜像后 **PASS** @ `:8196`
  - 清 tip `b3456e2` 残差（JoyCaption HF 仅在 `:8193`，池子却派到 `:8196`；已转设备镜像）
- **真源（勿 stage）**：`public_e2e_retest_011723_joycaption.json`（路径可能在 `.regen_tmp/...` — **仅引文件名，勿 stage**）。
- **前置 tip**：`b3456e2`（wave1 设备复测；pulid/removebg PASS；JoyCaption 8193）— 已为本地 parent。
- **相关**：wave1 设备复测 STATE `public_e2e_retest_wave1_device_2026_09_11`（011723 JoyCaption 残差 **已清**）。
- **硬口径**：**非** shipped/online；产品仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`live_on_core_public_e2e_011723_joycaption_pass`（非 shipped）；STATE `public_e2e_retest_011723_joycaption_2026_09_11`；`updated_at` 2026-09-11T06:43:00+08:00。via 项目管家（ToIV 开发）。

### 2026-09-11（项目管家：wave1 设备复测；pulid/removebg PASS；JoyCaption 8193）
- **wave1 设备复测（ToIV 开发；via 项目管家）**：
  - `pulid` / `removebg` **PASS**
  - `011723` JoyCaption HF：仅在 `:8193`，池子却派到 `:8196`
  - **已转设备镜像**
- **真源（勿 stage）**：`public_e2e_retest_wave1_device.json`（路径可能在 `.regen_tmp/...` — **仅引文件名，勿 stage**）。
- **前置 tip**：`6c72c16`（wave2 nodes2；三旧清；RMBG/HF timeout）— 已为本地 parent。
- **相关**：wave1 产品残差 STATE `public_e2e_wave1_product_leftovers_2026_09_11`（JoyCaption/pulid 曾为残差）。
- **硬口径**：**非** shipped/online；产品仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`live_on_core_public_e2e_wave1_device_handed_mirror`（非 shipped）；STATE `public_e2e_retest_wave1_device_2026_09_11`；`updated_at` 2026-09-11T06:16:00+08:00。via 项目管家（ToIV 开发）。

### 2026-09-11（项目管家：wave2 nodes2；三旧清；RMBG/HF timeout）
- **wave2 nodes2 复测（ToIV 开发；via 项目管家）**：
  - 三旧节点清：`MimicMotionGetPoses` / `DF_Int_to_Float` / `Florence2Run`
  - `214347` 缺 **RMBG**
  - `213776` / `417975` **HF ConnectTimeout**
  - **已转设备**
- **真源（勿 stage）**：`public_e2e_retest_wave2_nodes2.json`（路径可能在 `.regen_tmp/...` — **仅引文件名，勿 stage**）。
- **前置 tip**：`8b2eb2e`（wave2 节点复测；038828 PASS；新缺三节点）— 已为本地 parent。
- **相关**：wave2 nodes STATE `public_e2e_retest_wave2_nodes_2026_09_11`；wave2 STATE `public_e2e_wave2_2026_09_11`。
- **硬口径**：**非** shipped/online；产品仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`live_on_core_public_e2e_wave2_nodes2_handed_device`（非 shipped）；STATE `public_e2e_retest_wave2_nodes2_2026_09_11`；`updated_at` 2026-09-11T06:12:00+08:00。via 项目管家（ToIV 开发）。

### 2026-09-11（项目管家：wave2 节点复测；038828 PASS；新缺三节点）
- **wave2 节点复测（ToIV 开发；via 项目管家）**：
  - `038828` **PASS**
  - `213776` / `214347` / `417975` 新缺：`MimicMotionGetPoses`、`DF_Int_to_Float`、`Florence2Run`
  - `107403` **Preview-only**
- **真源（勿 stage）**：`public_e2e_retest_wave2_nodes.json`（路径可能在 `.regen_tmp/...` — **仅引文件名，勿 stage**）。
- **前置 tip**：`2dfb295`（LTX23 复测；缺模清；image_1 残差）— 已为本地 parent。
- **相关**：wave2 STATE `public_e2e_wave2_2026_09_11`。
- **硬口径**：**非** shipped/online；产品仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`live_on_core_public_e2e_wave2_nodes_retest`（非 shipped）；STATE `public_e2e_retest_wave2_nodes_2026_09_11`；`updated_at` 2026-09-11T05:58:00+08:00。via 项目管家（ToIV 开发）。

### 2026-09-11（项目管家：LTX23 复测；缺模清；image_1 残差）
- **LTX23 复测（ToIV 开发；via 项目管家）**：
  - app `2053122050`：**缺模清**
  - 新残差：`LTXVImgToVideoInplaceKJ` unexpected `image_1`
  - ToIV 开发 **正修**
- **真源（勿 stage）**：`public_e2e_retest_ltx23.json`（路径可能在 `.regen_tmp/...` — **仅引文件名，勿 stage**）。
- **前置 tip**：`5e2fe3d`（Qwen3-VL 复测；缺模清；022665/Preview-only）— 已为本地 parent。
- **硬口径**：**非** shipped/online；产品仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`live_on_core_public_e2e_ltx23_retest_fixing`（非 shipped）；STATE `public_e2e_retest_ltx23_2026_09_11`；`updated_at` 2026-09-11T05:47:00+08:00。via 项目管家（ToIV 开发）。
...
- **Dirty 修复已部署上 core（仍未 git commit）**：
  1. `_normalize_melband_roformer`：FS path → registry display name
  2. sampler 默认值回填
- **真源（勿 stage）**：`public_e2e_retest_melband_save_preview.json`（路径可能在 `.regen_tmp/...` — **仅引文件名，勿 stage**）。
- **前置 tip**：`9bda626`（公开 E2E wave2；36 卡 PASS11/FAIL22/TIMEOUT3）— 已为本地 parent。
- **硬口径**：**非** shipped/online；产品仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`live_on_core_dirty_melband_039434_pass`（非 shipped）；STATE `public_e2e_retest_melband_pass_2026_09_11`；`updated_at` 2026-09-11T05:26:00+08:00。via 项目管家（ToIV 开发）。
...
- **MelBand `039434` 残差**：清 model_names 问题后，残差 = **Preview-only / 主保存未产出**（仅 preview 路径；主保存未产出）。
- **真源（勿 stage）**：`public_e2e_retest_melband_quant.json`（路径 `.regen_tmp/...` — **仅引文件名，勿 stage**）。
- **当前**：ToIV 开发 **正查产品**（产品仍 dirty，**未** git commit）。
- **前置 tip**：`8451b54`（MelBand+WanVideo dirty 再部署；复测 039434/109907）— 已为本地 parent。
- **硬口径**：**非** shipped/online；产品仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`live_on_core_public_e2e_109907_pass_melband_preview_residual`（非 shipped）；STATE `public_e2e_retest_melband_quant_2026_09_11`；`updated_at` 2026-09-11T05:02:00+08:00。via 项目管家（ToIV 开发）。

### 2026-09-11（项目管家：MelBand+WanVideo dirty 再部署；复测 039434/109907）
- **Dirty 再部署上 core（ToIV 开发；via 项目管家）**：产品 **dirty re-deployed** 到 core（**仍未** git commit）：
  1. `MelBandRoFormerModelLoader` 加入 `model_names` + scan `vocal_separator` / `MelBandRoFormer` 目录
  2. WanVideo：非 scaled 权重 strip `_scaled` quantization
- **复测中**：`039434` / `109907`
- **前置 tip**：`93d74fa`（H3 r2v 三卡 PASS；109907 quantization `_scaled` 残差）— 已为本地 parent。
- **硬口径**：**非** shipped/online；产品仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`live_on_core_dirty_melband_wan_quant_retesting`（非 shipped）；STATE `public_e2e_melband_wan_quant_dirty_redeploy_2026_09_11`；`updated_at` 2026-09-11T04:51:00+08:00。via 项目管家（ToIV 开发）。
...
- **Dirty 部署上 core（ToIV 开发；via 项目管家）**：产品 **dirty deployed** 到 core（**未** git commit）。
  - H3 **AUTOGROW** ref mapping
  - Text Multiline **dynamic_prompts** backfill
- **复测中**：`h3-r2v` / `h3-r2v-voice` / `rh-2k-h3` + `109907`。
- **前置 tip**：`6856f2c`（MelBand 复测 FAIL；model_names 漏 Loader；正改 client.py）— 已为本地 parent。
- **硬口径**：**非** shipped/online；产品仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`live_on_core_dirty_h3_autogrow_dynamic_prompts_retesting`（非 shipped）；STATE `h3_autogrow_dynamic_prompts_dirty_deploy_2026_09_11`；`updated_at` 2026-09-11T04:42:00+08:00。via 项目管家（ToIV 开发）。

### 2026-09-11（项目管家：MelBand 复测 FAIL；model_names 漏 MelBandRoFormerModelLoader；正改 client.py）
- **MelBand 复测 FAIL（ToIV 开发；via 项目管家）**：MelBand 复测仍 **FAIL**。
- **根因**：FS 已有权重，但 core `model_names` **未包含** `MelBandRoFormerModelLoader` → pool **误判**缺模型。
- **真源（勿 stage）**：`public_e2e_retest_melband.json`（路径 `.regen_tmp/...` — **仅引文件名，勿 stage**）。
- **当前**：正改 `client.py` 的 `model_names`（产品 dirty，**未** git commit）。
- **前置 tip**：`39a536d`（dual-pushed）：wave1 节点复测；MelBandRoformer 曾在 wave1 **下载**分流。
- **硬口径**：**非** shipped/online；产品仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`live_on_core_public_e2e_melband_model_names_fix`（非 shipped）；STATE `public_e2e_retest_melband_2026_09_11`；`updated_at` 2026-09-11T04:37:00+08:00。via 项目管家（ToIV 开发）。
...
- **真源（勿 stage）**：`public_e2e_wave1.json`（路径可能在 `.regen_tmp/...` — **仅引文件名，勿 stage**）。
- **失败分流**：
  - **产品**：H3 `ref_image_1` ×3（及同类产品问题）
  - **设备**：CR Prompt Text、Load Multiline
  - **下载**：Qwen3-VL、MelBandRoformer
- **当前**：正修 **H3**。
- **前置 tip**：`092a7f8`（dual-pushed）：inventory 550；L2 12/12/526；wave1 ~20 started（现已扩至 36）。
- **硬口径**：**非** shipped/online；产品仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`live_on_core_public_e2e_wave1_done_fixing_h3`（非 shipped）；STATE `public_e2e_wave1_2026_09_11`；`updated_at` 2026-09-11T04:26:00+08:00。via 项目管家（ToIV 开发）。
...
- **规划基线**：软隐藏的 **hard-blocked** apps **不纳入**下一步规划基线。
- **公开全量真跑矩阵（开工）**：inventory → batched E2E → 失败分流至 **产品 / 设备 / 下载**。
- **前置 tip**：`2ac5a3b`（dual-pushed）：116926/140266 PASS；CUDA 残差清；LongCat WanAnimate 热路径两卡收口。
- **硬口径**：本规划/矩阵开工本身 **非** shipped/online；产品仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`roadmap_public_e2e_matrix_kickoff`（非 shipped）；STATE `public_e2e_pass_closeout_def_2026_09_11`；`updated_at` 2026-09-11T03:38:00+08:00。via 项目管家（ToIV 开发 · 用户锁定）。
...
- **复测**：`116926` / `140266` **进行中**
- **真源（待）**：`.regen_tmp/app_test_matrix_p0/l2_retest_after_sageattn_sdpa.json`（expected path，可能尚不存在；**勿 stage**）
- **前置 tip**：`89cd349`（HF DWPose/SAM2 已清；新残差 WanVideoSampler CUDA @ LongCat；dual-pushed）
- **硬口径**：**非** shipped/online；产品码仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`live_on_core_l2_sageattn_sdpa_fallback_retesting`（非 shipped）；STATE `l2_sageattn_sdpa_fallback_2026_09_11`；`updated_at` 2026-09-11T03:13:00+08:00。via 项目管家（ToIV 开发 tip）。

### 2026-09-11（项目管家：HF DWPose/SAM2 已清；新残差 WanVideoSampler CUDA @ LongCat）
- **结果（ToIV 开发；via 项目管家）**：HF DWPose/SAM2 后 L2 复测。
- **已清**：HF **DWPose** / **SAM2**
- **新残差**：两张 RH 卡共用 **WanVideoSampler** 错误 — `Tensor must be on CUDA`（on **LongCat**）
- **真源**：`.regen_tmp/app_test_matrix_p0/l2_retest_after_hf_dwpose_sam2.json`（**勿 stage**）
- **交接**：已交 **设备管家**
- **前置 tip**：`9eaeffc`（DWPose HF 无 local / SAM2 HF timeout；Int/ONNX/Wan 已清；dual-pushed）
- **硬口径**：**非** shipped/online；产品码仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`live_on_core_l2_retest_after_hf_dwpose_sam2`（非 shipped）；STATE `l2_retest_after_hf_dwpose_sam2_2026_09_11`；`updated_at` 2026-09-11T03:09:00+08:00。via 项目管家（ToIV 开发 tip）。

### 2026-09-11（项目管家：ONNX/Wan 后复测；DWPose/SAM2 残差；Int/ONNX/Wan 已清）
- **结果（ToIV 开发；via 项目管家）**：ONNX/Wan22path 后 L2 复测。
- **新残差**：
  - `116926`：**DWPose** — HF，无 local
  - `140266`：**SAM2** — HF timeout
- **已清**：Int / ONNX / Wan
- **真源**：`.regen_tmp/app_test_matrix_p0/l2_retest_after_onnx_wan22path.json`（**勿 stage**）
- **交接**：已交 **设备管家** 预缓存
- **前置 tip**：`e0706fc`（Int/Preview 修复 dirty 部署上 core；复测 140266/116926 中；dual-pushed）
- **硬口径**：**非** shipped/online；产品码仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`live_on_core_l2_retest_after_onnx_wan22path`（非 shipped）；STATE `l2_retest_after_onnx_wan22path_2026_09_11`；`updated_at` 2026-09-11T02:53:00+08:00。via 项目管家（ToIV 开发 tip）。
