 ...
 * ④ 样式:assistant-view.css(A3 外迁,原 style jsx global)+ av- 前缀 + 200ms 开合过渡;空态 Shift+Enter 提示
...
// A3(2026-09-22)组件工程化:composer→Composer.tsx,会话列表/抽屉/删除确认→SessionDrawer.tsx,
// 门户/popup 空态→PortalEmpty.tsx,styled-jsx 外迁 app/styles/assistant-view.css(值零变更)
const composerSrc = readSrc("components/assistant/Composer.tsx");
const drawerSrc = readSrc("components/assistant/SessionDrawer.tsx");
...
test("popup 会话抽屉:仅 popup 渲染 + 新会话置顶 + 列表复用 renderConvList", () => {
  assert.ok(drawerSrc.includes("av-pop-conv"), "抽屉容器 class 缺失");
  assert.ok(
    drawerSrc.includes('className={`av-pop-conv${historyOpen ? " is-open" : ""}`}'),
    "抽屉 is-open 开合逻辑缺失",
  );
  // 抽屉内:新会话按钮置顶(head 区) + 列表复用页形态同一份渲染
  const drawer = drawerSrc.slice(drawerSrc.indexOf("av-pop-conv-head"));
  ...
  assert.ok(drawer.includes("<ConvList"), "抽屉未复用会话列表组件");
  // A3:抽屉挂载于 composer(锚于输入框上方),且在 @ 技能面板之前
  const drawerIdx = composerSrc.indexOf("<SessionDrawer");
  assert.ok(drawerIdx >= 0, "composer 未挂载 popup 会话抽屉");
  assert.ok(
    drawerIdx < composerSrc.indexOf("av-skill-panel at-card"),
    "抽屉应在 @ 技能面板之前(同锚位)",
  );
  // 会话列表同时被页形态历史面板与 popup 抽屉使用(不复制两套列表)
  assert.ok(
    viewSrc.includes("<ConvList") && drawerSrc.includes("<ConvList"),
    "会话列表未在页面板+抽屉两处复用",
  );
});

test("renderConvList:加载态/空态/列表三分支 + 当前会话高亮 + 删除入口", () => {
  const fn = drawerSrc.slice(
    drawerSrc.indexOf("export function ConvList"),
    drawerSrc.indexOf("export interface SessionDrawerProps"),
  );
...
test("删除会话:二次确认 Modal → deleteConversation → DELETE API", () => {
  // Modal 已在组件根部(popup 不屏蔽),确认按钮触发删除(A3:弹窗组拆至 SessionDrawer.tsx)
  assert.ok(drawerSrc.includes("open={!!confirmDeleteConv}"), "删除确认 Modal 缺失");
  assert.ok(
    drawerSrc.includes("deleteConversation(confirmDeleteConv.id)"),
...
/* ── ④ 样式与空态提示 ── */
test("样式:assistant-view.css 外迁 + av- 前缀 + 200ms 开合过渡 + reduced-motion", () => {
  // A3:styled-jsx 外迁独立 CSS 文件,主壳 import 挂链(原 style jsx global 注入序等价)
  assert.ok(
    viewSrc.includes('import "@/app/styles/assistant-view.css"'),
    "主壳未挂接 assistant-view.css",
  );
  assert.ok(!viewSrc.includes("<style jsx"), "styled-jsx 应已外迁 assistant-view.css");
  assert.ok(viewCss.includes(".av-pop-conv {"), "抽屉样式缺失");
  assert.ok(viewCss.includes(".av-pop-conv.is-open"), "抽屉开启态样式缺失");
  const style = viewCss.slice(viewCss.indexOf(".av-pop-conv {"));
  ...
  assert.ok(
    viewCss.includes(".av-pop-conv { transition: none; }"),
    "reduced-motion 未豁免抽屉过渡",
  );
  // 关闭态不可交互
  assert.ok(/\.av-pop-conv \{[^}]*pointer-events:\s*none/s.test(viewCss), "关态未禁指针事件");
});

test("popup 空态:Shift+Enter 唤起/关闭提示", () => {
  const empty = portalSrc.slice(
