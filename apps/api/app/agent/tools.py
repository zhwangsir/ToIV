...
H2 起工具注册/守卫管线收进 harness 工具缝(app.harness.tool_seam,ctx.tools):
8 个执行器(exec_*)为本文件的事实源,tool_seam 薄包装注册;execute() 保留
兼容入口,委托注册表执行(带 TOOLS_PRE_EXECUTE 守卫 / TOOLS_POST_EXECUTE 审计)。
"""
...
# ---------------------------------------------------------------------------
# 8 个内建工具执行器(由旧 execute 的 if 分支逐字搬出,行为以既有测试为锁)。
# 统一签名:(args, pool, user, session, attachment) → (给 LLM 的文本, 媒体事件)。
# tool_seam 以此注册 ToolSpec;新增工具请走 tool_seam 注册,不要在本文件加分支。
# ---------------------------------------------------------------------------
...
async def exec_generate_video(args: dict, pool: WorkerPool, user: User, session, attachment: dict | None = None) -> tuple[str, list[dict]]:
    settings = get_settings()
    prompt = args["prompt"]
...
async def exec_edit_image(args: dict, pool: WorkerPool, user: User, session, attachment: dict | None = None) -> tuple[str, list[dict]]:
    settings = get_settings()
...
async def exec_generate_3d(args: dict, pool: WorkerPool, user: User, session, attachment: dict | None = None) -> tuple[str, list[dict]]:
    settings = get_settings()
...
async def execute(
    ...
    """兼容入口(H2 前的外部调用方):委托 harness 工具注册表执行(带守卫管线)。

    未知工具与旧行为一致:返回 "未知工具: xxx" 文本,不抛异常。
    """
    registry = get_ctx().service("tools")
