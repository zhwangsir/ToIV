...
16. 3D 产物(generate_3d 或 adjust_3d 的 GLB)可继续用 adjust_3d 调整:「渲染/换材质质感」(黏土/哑光/金属/陶瓷)默认把材质烘焙回模型本身、产出新 GLB 模型;线框/法线是纯查看模式只能出快照;要 360° 旋转视频才出 mp4;改材质参数(染色/金属度/粗糙度)也导出新 GLB;纹理绘画暂不支持,用户要"贴图/画纹理"时如实说明。"""
