...
LOG = EVAL_DIR / "r2v_eval_a4_result.log"
...
def t2v_prompt(text: str, length: int, seed: int) -> dict:
    ...
    for n in base.values():
        if n["class_type"] in ("MiniMaxH3TextToVideo", "MiniMaxH3ImageToVideo"):
...
def main() -> None:
    ...
    fetch_artifacts(entry, "a4_portrait_src")
    # 2) 抽中间帧
    subprocess.run(["ffmpeg", "-y", "-ss", "1.2", "-i", str(LOCAL_OUT / "a4_portrait_src.mp4"),
                    "-frames:v", "1", str(EVAL_DIR / "a4_portrait_ref.png")],
                   check=True, capture_output=True)
    # 3) 上传
    boundary = uuid.uuid4().hex
    body = (f"--{boundary}\r\n".encode()
            + b'Content-Disposition: form-data; name="image"; filename="a4_portrait_ref.png"\r\n'
            + b"Content-Type: image/png\r\n\r\n"
            + (EVAL_DIR / "a4_portrait_ref.png").read_bytes() + b"\r\n"
            + f"--{boundary}--\r\n".encode())
    ...
    log(f"a4 r2v: {pid}")
    entry, dt, peak = wait_done(pid)
    log(f"a4_portrait_kitchen: {'done' if entry.get('status',{}).get('completed') else 'ERROR'} {dt:.0f}s 峰值 {peak}")
    fetch_artifacts(entry, "a4_portrait_kitchen")
    log("=== A4 完成 ===")
