...
> 2026-08-14 补：引擎选型已有真机实证结论——「精确轨迹/运镜控制」类需求路由 H3，LTX-2.5 走通用 SFW 音画同出（→见第八节）。
...
### 8.1 精确轨迹/运镜控制：H3 显著强于 LTX-2.5

真机对比（同一「精确轨迹 + 静态背景」类 prompt：鲨鱼沿椭圆轨迹游动且背景静止）：

- **LTX-2.5（:8198）**：轨迹漂移 + 背景漂移，对「精确轨迹 + 静态背景」的遵循能力弱，不满足运镜级控制需求。
- **MiniMax H3（:8195）**：用**分阶段路径描述 + 背景稳定约束**的提示词写法，可达成平滑椭圆轨迹 + 静态背景 + 无缝循环。实证参数：226 帧 9.42s@24fps，seed `7358494407347417035`，steps=20，res_multistep + simple。

**路由结论**：精确运镜/轨迹控制类需求一律路由到 **H3**；**LTX-2.5 定位通用 SFW 音画同出**主力引擎，不承接精确轨迹控制。

### 8.2 H3 帧网格 17k+5 无法精确命中整秒

H3 帧数网格为 `17k+5`：10s@24fps = 241 帧**不在网格上**，实际最近落点为 226 帧（17×13+5）= 9.42s。产品侧时长预设须按网格换算后再展示（如「10s」实际交付 9.42s），避免用户预期与产物时长不符。
