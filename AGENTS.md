...
| **数字人 M1-M3**(2026-08-27) | ✅ 形象库(ReferenceAsset kind=avatar+green_screen+ref_audio)+ avatar/talk **TTS 直通**(drive_text/voice/speed 与 audio 互斥,TTS 失败 502 零半成品)+ ASR→SRT(`GET /api/dub/transcribe/{id}?format=srt`);借鉴 aigcpanel(形象模板/双驱动/字幕导出),M4 通用对口型矩阵(MuseTalk 系)待评估,智能直播不采纳 |
