...
- 本地 `e833f33`（未推）：助手上下文溢出（用户贴的 NSFW LLM 主备均不可用 / 32768 token / 0 output tokens）。工作副本 tool 正文上限 1800 字；`chat()` 不再每轮塞 17 个 `mcp__` schema；超长 400 再压一半预算重试一次，仍失败中文提示「对话太长，模型上下文已满。请新开一个会话，或删掉本轮过长的附件后再试。」NSFW 与主模型同一端点时不再双打。落库 `AgentMessage` 仍全量。生产还没有这版。
