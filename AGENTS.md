...
- **用户确认全序（ToIV 开发；via 项目管家）**：
  1. **P0** 分级实测 — 开发**已开工 P0 L0**（status=`in_progress`，**非** shipped/online）。
  2. **P1** 应用说明卡（用法/作用）。
...
- **硬口径**：产品实现**未 commit** — 标 `planned` / `in_progress`（P0 L0），**禁止**标 shipped/online；产品码仍 dirty **勿 stage**；勿 stage `.regen_tmp`；无产品 SHA；本地 tip **无 push**。
- Status：`roadmap_confirmed`（P0 L0=`in_progress`；其余=`planned`；**非 shipped**）；STATE `roadmap_p0_p1_p3_p2_2026_09_11`；`updated_at` 2026-09-11T00:37:00+08:00。via 项目管家（ToIV 开发 tip）。
