...
> **最后更新**：2026-09-11（项目管家：wave1 设备复测；pulid/removebg PASS；JoyCaption 8193）：wave1 设备复测 — `pulid` / `removebg` **PASS**；`011723` JoyCaption HF 仅在 `:8193`，池子却派到 `:8196`；**已转设备镜像**；真源 `public_e2e_retest_wave1_device.json`（`.regen_tmp/` — **仅引文件名，勿 stage**）；前置 tip `6c72c16`（wave2 nodes2）— 本地 parent；相关 wave1 产品残差（JoyCaption/pulid 曾为残差）；产品 dirty **勿 stage** / `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_wave1_device_handed_mirror`；STATE `public_e2e_retest_wave1_device_2026_09_11`；`updated_at` 2026-09-11T06:16:00+08:00；via 项目管家（ToIV 开发）；
