...
- **🟢 09-21 D 组小件双核销 + C 组模型收口（api 3298 基线不变）**：①**builtin 封面核查=已完工**——PG 真核：公开非 R18 应用无封面数=**0**（895 个 builtin 无封面全部为非公开/R18,demo 管线按设计不覆盖;`planned=0` 不是 bug);②**SFW/NSFW 合并残留=运营层已并,注册表去重按低值/高风险跳过**——前端 `STUDIO_MODES` 已把 nsfw 孪生并入同槽（txt2img+nsfw-txt2img 同 t2i 槽、ltx-nsfw-t2v 与 SFW 同 t2v 槽）,后端按 R18 上下文混入/剔除;注册表层再并 ltx/wan-nsfw 属纯装饰性去重,撞 24 引擎探测/测试矩阵风险不值,决策跳过入册;③**C 组模型**——142022 三件套复核全在位（darkBeast dbkleinv2BFS/flux2-vae 双目录/qwen_3_8b 15.3GiB,残旧核销）;**`minimax_h3_ref2va_pruned_bf16` 37.45GiB 已落盘**（hf-mirror aria2 -x16 ~334MiB/s,safetensors 魔数核,:8195 重启后 **UNETLoader 在列**;H3 pruned 质量/体积折中档补齐,bf16 全量/fp8/int8/pruned_bf16 四档齐）;ltx spatial 随 LTX-2.5 退役核销;MODEL_SOURCES **ok476/blocked337/total813**（MD/JSON 双更）。⚠️ 易错点：core 上 `toiv/api/toiv.db` 不是生产库（生产=PG `toiv@127.0.0.1:5432/toiv`,直查用 psql+deploy/.env 密码）。
- **🟢 09-21 B 组设备包清理（commit `02d3df4`，api 3298 全绿）**：盘点发现 B 组大半残旧已核销（SeedVR2 权重 8 文件齐于 `/opt/ComfyUI/models/SEEDVR2`(NAS 链接,145828 应用 DB 已 pass)/LayerMask `segformer_b2/b3_clothes` 齐/`models/prompt_generator` 五模型齐/Easy-Sam3·IPAdapter·was-node 在）；**SeC 387721 链三修（真机五轮烟测逐层打通）**：①`_MISSING_REQUIRED_DEFAULTS` 补 `SeCModelLoader.model_file=SeC-4B-fp16.safetensors` 且**回填语义扩到「null 视同缺失」**（RH 图常带 null 而非缺键）;②**`:8197(longcat)补 SeC 权重软链**——SecNodes 约定 `models/sams/`（包 `__init__.py` 自注册 folder_paths）,NAS `toiv/comfyui-models/sams/SeC-4B-fp16+bf16` 软链入（:8195 同款 09-17 已有；:8196 fp16 已见；:8199 无 SecNodes 类）;③`_normalize_sec_empty_bbox`(`bbox ""`→None——SecNodes `parse_bbox` 对 None 早退、对 `""` 误入 `__getitem__` dict 分支炸 `string indices must be integers`);④`_normalize_sec_flash_attn_blackwell`(`use_flash_attn` True→False)。推进轨迹：validation ✓→missing_model ✓→bbox ✓→模型加载运行 ✓;**剩余 einops reshape 错（`B (L S) H D` @ [1,17160,40,128]）复测证明与 flash-attn 无关,属 SecNodes 模型内部×本环境（torch 2.13+cu130/sm_120）兼容问题,登记上游边界**（flash-attn 关闭保留——Blackwell eager 更稳语义一致）。⚠️ 易错点：SecNodes 权重目录约定=`models/sams` 而非 LLM;**「存在但 null」与「缺键」是同坑两态,回填器须两态并查**。
