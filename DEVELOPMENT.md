...
**2026-08-30 `eb51c86`（本地未推，尚未上线）**：本地 `eb51c86`（未推）：停止会 `cancelJob`/中止请求；主体封面走 i2v/img2img；时长 4–15s 加分段续写开关；作品库含 `h3_extend_i2v` 和 `cad_`/`drama_char_reference_` 前缀。正在部署 core，**尚未上线**。远程未推。
