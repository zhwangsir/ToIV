...
**2026-09-08 LIVE on core dirty：capability gap 收口（ToIV 开发；无产品 SHA）**：软隐藏 RHHiddenNodes+裸 Text union≈100 → `is_public=false`（DB 公开≈982/非公开≈1313/总≈2295）；`_pick_app_client` 扩路由已 LIVE（`--skip-web`，api health OK）——H3→`192.168.71.127:8195`（`TOIV_H3_BASE_URL`）；QwenEdit→默认 `192.168.71.114:8194`（无显式 `TOIV_QWEN_EDIT_BASE_URL`）；能力扫描 OK v1 58→v8 163（+105）；下一波见 `.regen_tmp/capability_gap_scan_v8/actionable/next_wave_misc_packages.md`；通用池 workers 仍 `127:8196`/`116:8188`/`114:8193`；勿 stage `.regen_tmp`/产品 dirty/`rh_h3_presets.json`。status=`live_on_core_dirty`；STATE `capability_gap_soft_hide_dedicated_pool_2026_09_08`；updated_at 2026-09-08T21:29:00+08:00。
