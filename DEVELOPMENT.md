...
**2026-09-11 L2 路由 dirty 部署；抽测4/4 improved、pass0（项目管家；ToIV 开发 tip；无产品 SHA）**：live_core 经 `deploy.sh --skip-web`（~01:27 CST）部署 L2 路由修复；**产品仍未 git commit**（dirty deploy only）。抽测 **4/4** `routing_improved=true`、**pass=0**。残差：缺节点 **3** class_type（Bjornulf_ShowFloat / PoseAndFaceDetection / PixelPerfectResolution）+ wan-animate ffmpeg/fixture。真源 `.regen_tmp/app_test_matrix_p0/l2_retest_after_routing.json` **勿 stage**；已交 **设备管家** 装缺节点。**非** shipped/online（仅 dirty-deployed）；产品 dirty 勿 stage；无产品 SHA；本地 tip 无 push。status=`live_on_core_dirty_routing_improved_pass0`；STATE `l2_routing_dirty_deploy_retest_2026_09_11`；updated_at 2026-09-11T01:30:00+08:00。
...
**2026-09-11 拍板 P0实测→P1说明卡→P3扩展Admin→P2说明书KG（项目管家；ToIV 开发 tip；无产品 SHA）**：用户确认全序 — **P0** 分级实测 → **P1** 应用说明卡（用法/作用）→ **P3** 万能管理台（扩展现有 Admin，非另起后台）→ **P2** 说明书知识图谱。~~本 tip `ae2b868` 曾写 P0 L0=`in_progress`~~ → **已纠正**：P0 L0 **550/550 pass**（见新 tip / STATE `p0_l0_pass_2026_09_11`）。**非 shipped/online**（产品实现未 commit；仅 L0 结果完成）。旁证：AdminView=users/agents/audit only；最小 KG `GET /api/admin/knowledge-graph` ≈1295/608、无 howto；公开 ≈550（video359/image189），描述 p50≈40。产品 dirty 勿 stage；无产品 SHA。status=`roadmap_confirmed`→P0 L0 现 `pass`（其余=`planned`）；STATE `roadmap_p0_p1_p3_p2_2026_09_11`；原 updated_at 2026-09-11T00:37:00+08:00（L0 纠正见 2026-09-11T00:44:00+08:00）。
...
**2026-09-10 v20 fill节点≈27/30class + MODEL_SOURCES 467/343/810 + v21 unhide×36 →546（项目管家；设备管家+模型下载+开发/能力缺口 tip；无产品 SHA）**：①设备管家：通用池 v20 fill 节点包约 **27**；三机齐 **30** class_type（明细以设备→开发为准，**勿编造包名**）。②MODEL_SOURCES v20 unhide 后 ok**467**/blocked**343**/total**810**（前 465/339/804；Δ +2/+4/+6）；newly_ok **2** ≈51.23 GiB（10Eros_Max_h3_TURBO-hybrid_beta3_int8_convrot + minimax_h3_ref2va_int8_convrot → `h3/diffusion_models/`）；同批 blocked 新记 4；摘要 `.regen_tmp/capability_gap_v20_unhide_download_summary.json` **勿 stage**。③capability_gap_scan_v21：主 OK **454**（vs v20 245）；dedicated_aware **1600**（+animate2 **1614**）；批量 unhide **36**（P0 nodes→OK **6** + 另 **30**；fail=0）公开约 **510→546**；仍等下载 **47** / 仍缺节点 **27**；真源 `.regen_tmp/capability_gap_scan_v21/actionable/unhide_ready_after_v21.json` **勿 stage**。产品码仍 dirty 勿 stage；无产品 SHA。status=`docs_consolidated`；STATE `v20_fill_nodes_model_sources_v21_unhide_2026_09_10`；updated_at 2026-09-10T17:02:00+08:00。

**2026-09-10 animate2 unhide 已落地 508→510；admin soft-hide 可再上架（项目管家；ToIV 开发+能力缺口 tip；产品 dirty 待 commit）**：产品码 **仍 dirty 未 commit**（status=`pending_product_commit`）— `apps.py` admin `_visible`+PUT 再上架；`app_seed.py` seed **不覆盖**已有 `is_public`；`test_admin_can_unhide_soft_hidden_catalog_and_builtin`；**勿 stage**。**生产已落地**：core 两轮 `deploy.sh --skip-web`（admin soft-hide 可再上架 + seed 不覆 is_public）；animate2 dedicated unhide — `wan-animate-2` + `rh-character-replacement-minimax-h3-7582cc8e`（WanAnimate2ToVideo；`:8199`）→ 公开 **508→510**（不是待 unhide）；重启误开 `avatar-talk`/`facedetailer`/`hunyuan-i2v`/`latentsync` 已再 soft-hide。能力缺口：通用 P0 **80** 仍在 `unhide_after_fill_p0.json` 等 fill；animate2×2 已从候选变为已 unhide。v20 旁路已齐：`dedicated_aware_ok(H3)=510`；`ok_if_dedicated_animate2=2`（均已 unhide）；`with_animate2=512`；真源 `dedicated_aware.json` + `actionable/dedicated_aware_animate2_v20.json` **勿 stage**；**v21+ 必跑 animate2 postprocess**。产品公开约 **510**；勿 stage `.regen_tmp`/产品 dirty；无产品 SHA。STATE `admin_soft_hide_relist_wan_animate2_2026_09_10`；updated_at 2026-09-10T15:55:00+08:00。

**2026-09-10 mass soft-hide unusable v20 ×388（项目管家；ToIV 开发 tip；无产品 SHA）**：用户拍板只留能用；清单 hide **398**/新藏~**388**；生产 soft-hide **388** → `is_public=false`；公开约 **896→508**（期望 510；API list_len 510，含 2 张非公开 keep）；keep=main_ok∪ok_if_dedicated_h3=**510**（245+265）；验收 `post_soft_hide_verify_v20` **ok**：公开 **508** ⊆ keep **510**；漏网 **0**；keep−public=2；清单398−生产388=10 已藏硬阻-only，无补差；**不全量 v21**；真源 `.regen_tmp/capability_gap_scan_v20/actionable/soft_hide_all_non_usable_v20.json`（清单）+ `soft_hide_all_unusable_v20.json`/`_applied.json`（生产）+ `post_soft_hide_verify_v20.json`（验收；**勿 stage**）；不列 388 ids；产品 dirty 勿提交；未写 h3/；无 MODEL_SOURCES 变更。status=`soft_hide_docs`；STATE `soft_hide_all_unusable_v20_2026_09_10`；updated_at 2026-09-10T14:22:00+08:00。

**2026-09-10 soft-hide hard-block-only v20 ×10（项目管家；ToIV 开发 tip；无产品 SHA）**：仅硬阻、无缺节点 **10** 张 → `is_public=false`；公开约 **908→898**；ids `rh-acc-0951611393-511ac0` / `rh-acc-1075350528-dc3518` / `rh-acc-1796676609-bd7f4d` / `rh-acc-2769283074-6244b3` / `rh-acc-3253770242-2babaa` / `rh-acc-5038745602-15deeb` / `rh-acc-5875539969-0b9198` / `rh-acc-6473673730-f7ba44` / `rh-acc-8106374145-615ed6` / `rh-acc-9431956482-7fa928`（短：511ac0 dc3518 bd7f4d 6244b3 2babaa 15deeb 0b9198 f7ba44 615ed6 7fa928）；规则=missing_nodes 空且缺模全 hard-blocked/no-source（exclude already soft-hidden / H3-node residuals）；真源 `.regen_tmp/capability_gap_scan_v20/actionable/soft_hide_hard_block_only_v20.json` **勿 stage**（已 applied）；不扫 v21；放宽版不需要；Gitee main `dae4ee8`；GitHub 仍不可达；未写 h3/；无 MODEL_SOURCES 变更。status=`soft_hide_docs`；STATE `soft_hide_hard_block_only_v20_2026_09_10`；updated_at 2026-09-10T14:05:00+08:00。

**2026-09-10 capability_gap_scan_v20 完成（项目管家；ToIV 能力缺口 tip；无产品 SHA）**：apps **908**（−1 soft-hide ef8fd7，对齐 02737db；vs v19 909）；OK **245（持平 vs v19，+0）**；blocked **663**；dedicated-aware **510（0）**；only_h3 **265**；worker models：8196=**838** / 8188=**822** / 8193=**822**；style_models=**2**；loras=**337**；ef8fd7 已从公开/blocked 消失；主 OK 持平；等长尾或产品拍板再扫；仍缺 Top 非 H3：银月/燕如嫣/梅凝/韩立南宫婉/peiling3(6) + model.pt(5) + Kook亚洲人像/Licon-MSR/bfs/new_flux-2-klein-9b(4)；Top 缺节点仍 H3/RH + TT/Comfly；真源 `.regen_tmp/capability_gap_scan_v20/` 勿 stage。status=`scan_complete_docs`；STATE `capability_gap_scan_v20_2026_09_10`；updated_at 2026-09-10T13:50:00+08:00。

**2026-09-10 soft-hide `rh-acc-1004064770-ef8fd7`（项目管家；ToIV 开发 tip；无产品 SHA）**：Z-image真实感-电商高清文生图 → `is_public=false`；公开约 **909→908**；原因=仅缺 hard-blocked `beyondREALITY_beyondREALITYZIMAGE.safetensors`（UltimateSDUpscale 已清 / v19 node gap）；更新 24ae742「勿藏 ef8fd7」口径；同权此前 `rh-acc-8851286018-43497c` 已藏（910→909）；真源 `.regen_tmp/capability_gap_scan_v19/actionable/soft_hide_beyondreality_zimage_followup.json` **勿 stage**；未写 h3/；无 MODEL_SOURCES 变更。status=`soft_hide_docs`；STATE `soft_hide_ef8fd7_beyondreality_followup_2026_09_10`；updated_at 2026-09-10T13:46:00+08:00。

**2026-09-10 capability_gap_scan_v19 完成（项目管家；ToIV 能力缺口 tip；无产品 SHA）**：apps **909**（post soft-hide 8851286018；vs v18 910）；OK **245（+1 vs v18）**；blocked **664**；dedicated-aware **510（+1）**；only_h3 **265**；worker models：8196=**838** / 8188=**822** / 8193=**822**；style_models=**2**；loras=**337**；已清 Remix high/low + moodyRealMix + BEYOND；UltimateSDUpscale 清（node gap；still_in_missing_nodes=[]）；仍缺 Top 非 H3：银月/燕如嫣/梅凝/韩立南宫婉/peiling3(6) + model.pt(5) + Kook亚洲人像/Licon-MSR/bfs/new_flux-2-klein-9b(4)；Top 缺节点仍 H3/RH + TT/Comfly；真源 `.regen_tmp/capability_gap_scan_v19/` 勿 stage。status=`scan_complete_docs`；STATE `capability_gap_scan_v19_2026_09_10`；updated_at 2026-09-10T07:21:00+08:00。

**2026-09-10 MODEL_SOURCES v17 长尾齐：ok465/blocked339/total804（项目管家；ToIV 开发+模型下载；无产品 SHA）**：前 tip 0616f0c 462/342/804（Δ +3/−3；若 BEYOND 已计则相对 0f6a0a4 量级 +4/−4）；newly_ok≈**49.55 GiB**：Wan2.2_Remix_NSFW high+low / moodyRealMix_zitV5DPO / `BEYOND REALITY SUPER Z IMAGE 3.0 淡妆浓抹 BF16`；`beyondREALITY_beyondREALITYZIMAGE.safetensors` **blocked**（无精确公开源；**勿**冒充淡妆浓抹 BF16）；soft-hide `rh-acc-8851286018-43497c` → `is_public=false`；市场约 **910→909**；**勿**藏 `rh-acc-1004064770-ef8fd7`（还缺 UltimateSDUpscale）；**未**写 `h3/`；header 已同步 JSON `updated_at` 2026-09-10T07:14:32+08:00；真源 `.regen_tmp/capability_gap_v17_download_summary.json` / `beyondreality_zimage_trace.json` / `capability_gap_scan_v18/actionable/soft_hide_beyondreality_zimage_only.json` **勿 stage**。另：设备管家 — 主池三机已装 `ComfyUI_UltimateSDUpscale`（`ssitu/ComfyUI_UltimateSDUpscale` @ `a5547db9`；submodule Coyote-A @ `2322caa`；**无 pip**）；object_info 有 UltimateSDUpscale / CustomSample / NoUpscale / Guider；**未** soft-hide `rh-acc-1004064770-ef8fd7`（仅记节点已装，可能日后 unblock）。status=`inventory_and_soft_hide_docs`；STATE `model_sources_v17_beyondreality_soft_hide_2026_09_10`；updated_at 2026-09-10T07:18:00+08:00。

**2026-09-10 capability_gap_scan_v18 完成（项目管家；ToIV 能力缺口 tip；无产品 SHA）**：apps **910**；OK **244（持平 vs v17，+0）**；blocked **666**；dedicated-aware **509（0）**；only_h3 **265**；worker models：8196=**838** / 8188=**822** / 8193=**822**；style_models=**2**；loras=**337**；已清：BEYOND REALITY SUPER Z IMAGE 3.0 淡妆浓抹 BF16（complete 12309881296 B；MODEL_SOURCES BR BF16 + worker 可见）；主 OK 持平（v17 已不在 missing；卡上他缺）；仍缺 Top 非 H3：银月/燕如嫣/梅凝/韩立南宫婉/peiling3(6) + model.pt(5) + Kook亚洲人像/Licon-MSR/bfs/new_flux-2-klein-9b(4)；Top 缺节点仍 H3/RH + TT/Comfly；真源 `.regen_tmp/capability_gap_scan_v18/` 勿 stage。status=`scan_complete_docs`；STATE `capability_gap_scan_v18_2026_09_10`；updated_at 2026-09-10T06:43:00+08:00。

**2026-09-10 MODEL_SOURCES：ok462/blocked342/total804（项目管家；ToIV 模型下载；BR BF16 落地）**：BEYOND REALITY SUPER Z IMAGE 3.0 淡妆浓抹 BF16 → `diffusion_models/`；**12309881296** B（≈11.46 GiB；ok）；自 v16 长尾续下已落地（此前「续下中未计入」）；header 已同步 JSON `updated_at` 2026-09-10T06:37:18+08:00（前 tip 0f6a0a4 461/343/804；Δ ok+1 / blocked−1）；**未**写 `h3/`；真源 `.regen_tmp/capability_gap_v16_download_summary.json` 勿 stage。status=`inventory_updated`；STATE `model_sources_br_bf16_2026_09_10`；updated_at 2026-09-10T06:40:00+08:00。


**2026-09-10 capability_gap_scan_v17 完成（项目管家；ToIV 开发 tip；无产品 SHA）**：apps **910**（post soft-hide）；OK **244（+2 vs v16）**；blocked **666**；dedicated-aware **509（+2）**；only_h3 **265**；worker models：8196=**835** / 8188=**819** / 8193=**819**；style_models=**2**；loras=**337**；已清 missing 4：SESELAORUYAO / DarkKlein9b_v2BFS_extracted_lora_r256 / Kook_Qwen_V3极致真实 / Kook_Zimage_如梦似幻（v16 下载长尾）；仍缺 Top 非 H3：银月/燕如嫣/梅凝/韩立南宫婉/peiling3(6) + model.pt(5) + Kook亚洲人像/Licon-MSR/bfs/new_flux-2-klein-9b(4)；Top 缺节点仍 H3/RH + TT/Comfly；BEYOND REALITY 续下中 incomplete；真源 `.regen_tmp/capability_gap_scan_v17/` 勿 stage。status=`scan_complete_docs`；STATE `capability_gap_scan_v17_2026_09_10`；updated_at 2026-09-10T06:10:00+08:00。

**2026-09-10 MODEL_SOURCES：ok461/blocked343/total804（项目管家；ToIV 模型下载；v16 非 H3 长尾一批）**：v16 长尾一批后计数对齐；header 已同步 JSON `updated_at` 2026-09-10T05:44:41+08:00（前 tip 7617fd4 457/347/804；Δ ok+4 / blocked−4）；newly_ok **4** ≈1.94 GiB（SESELAORUYAO / DarkKlein9b_v2BFS_extracted_lora_r256 / Kook_Qwen_V3极致真实 / Kook_Zimage_如梦似幻）/ already_on_nas **10** / P0/P1 blocked 含：亚洲人像/3kman-reface/ComfyUI_00002_/add_real_details/detail-new/h-anime4；BEYOND REALITY 续下中未计入；**未**写 `h3/`；真源 `.regen_tmp/capability_gap_v16_download_summary.json` 勿 stage。status=`inventory_updated`；STATE `model_sources_v16_non_h3_2026_09_10`；updated_at 2026-09-10T06:06:30+08:00。
...
**2026-09-10 MODEL_SOURCES：ok454/blocked350/total804（项目管家；ToIV 模型下载；v14 非 H3 续下）**：v14 非 H3 续下后计数对齐；header 已同步 JSON `updated_at` 2026-09-10T04:40:53+08:00（前 tip d2816ac 447/357/804；Δ ok+7 / blocked−7）；newly_ok **7** / already_on_nas **13** / blocked_documented≈**20**（含 Licon-MSR-V1_B / new_flux-2-klein-9b / bfs…rank128B）/ ≈39.95GiB；**未**写 `h3/`；真源 `.regen_tmp/capability_gap_v14_download_summary.json` 勿 stage。status=`inventory_updated`；STATE `model_sources_v14_non_h3_2026_09_10`；updated_at 2026-09-10T04:42:30+08:00。

**2026-09-10 capability_gap_scan_v14 完成（项目管家；ToIV 开发 tip；无产品 SHA）**：apps **924→922**（软隐藏 −2）；OK **224→235（+11）**；blocked 687；dedicated-aware **491→500（+9）**；only_h3 **265**；worker models：8196=**817** / 8188·8193=**798**；已清 P0/P1：FullDynamic_Ultimate_Fusion_Elite / Kook_Zimage_瑶光 / lightx2v_elite_it2v_animate_face / Flux2-Klein-9B-True-v2-bf16 / WAN22_MoCap_fullbodyCOPY_ED / Wan2.2-Fun-A14B-InP-Fusion-Elite；仍缺命名：LTX-2.3-Licon-MSR-V1_B(7)、new_flux-2-klein-9b(7)、bfs_head…rank128B(5)、gimmvfi_r_arb_lpips_fp32(7，疑 folder，已催设备)；新 Top 非 H3 缺模 ≤7 apps；Top 缺节点仍 H3/RH + TT/Comfly；真源 `.regen_tmp/capability_gap_scan_v14/` 勿 stage。status=`scan_complete_docs`；STATE `capability_gap_scan_v14_2026_09_10`；updated_at 2026-09-10T04:32:00+08:00。
...
**2026-09-10 LIVE：通用池 extrapaths 补 LLM/vocal_separator/MelBandRoFormer + llama-cpp 勿覆 LLM（设备管家）**：通用池 `extra_model_paths` 已加 `LLM` / `vocal_separator` / `MelBandRoFormer`；并 patch llama-cpp 勿覆盖 LLM 路径。解开 v13 still_missing 中似 folder-mount 项（MelBandRoformer_fp16 / Qwen3.5-9B* 相关；ltx spatial 或仍另轨）。**未**扫 v13b——仅记 mount/路径修复。旁证：~~T8前 only_h3 267→h3_ok 208 / still 59（deferred）~~ → **T8后侧扫** 主 212/55、别名 265/2（见新 tip `h3_8195_side_scan_t8`）；原 N3 中 2 不挡、T8 turbo 挡 1；结案仍 Y34/N3；真源旧 `.regen_tmp/.../h3_8195_side_scan/` + 新 `.../h3_8195_side_scan_t8/` 勿 stage。status=`live_device_fyi`；STATE `genpool_extrapaths_llm_vocal_melband_2026_09_10`；updated_at 2026-09-10T04:08:00+08:00。


**2026-09-10 LIVE：H3 `:8195` 复验结案 + H3补齐关闭（ToIV 开发）**：Y**34**/N**3**（+14 vs 初验 Y20/N17）；latent bf16+fp16 皆 Y；剩 3 无源 blocked 结案；新下 **13** OK ~121GiB → `h3/`；counts.blocked 摘要含 1 skip-already-present，真无源=3。通用池 folder mount 已补 LLM/vocal_separator/MelBandRoFormer（见新 tip）；ltx spatial 或仍另轨；**未**扫 v13b。真源 `.regen_tmp/.../h3_8195_audit_live_reverify.json` + `h3_8195_download_summary.json` 勿 stage。status=`h3_buqi_reverify_closed`；STATE `h3_8195_reverify_2026_09_10`；updated_at 2026-09-10T03:56:00+08:00。
...
**2026-09-10 capability_gap_scan_v13 完成（ToIV 开发；无产品 SHA）**：apps 924；OK **206→210（+4）**；blocked 714；dedicated-aware **457→477（+20）**；only_h3 **251→267**；worker models：8196=**760** / 8188·8193=**711**（各+10）；已清 `qwen3-vl-32b-int8_convrot`、`Kook_Zimage_真实幻想_Turbo`；仍缺 P0/P1 多为 folder mount → 通用池 extrapaths 已补（见新 tip），**未**扫 v13b；真源 `.regen_tmp/capability_gap_scan_v13/` 勿 stage。status=`scan_complete_docs`；STATE `capability_gap_scan_v13_2026_09_10`；updated_at 2026-09-10T03:56:00+08:00。
...
**2026-09-10 MODEL_SOURCES：ok412/blocked375/total787（ToIV 模型下载；RMBG-2.0）**：RMBG-2.0 落入后计数对齐；header 已同步 JSON `updated_at` 2026-09-10T03:13:45+08:00。落盘 `NAS/toiv/comfyui-models/RMBG/RMBG-2.0/model.safetensors` 884878856B；设备核 folder 已齐（见新 tip），v12 已扫完（见新 tip）。status=`inventory_updated`；STATE `model_sources_rmbg20_2026_09_10`；updated_at 2026-09-10T03:16:00+08:00。
...
**2026-09-08 LIVE on core dirty：capability gap 收口（ToIV 开发；无产品 SHA；H3 二次扩）**：软隐藏 RHHiddenNodes+裸 Text union≈100 → `is_public=false`（DB 公开≈982/非公开≈1313/总≈2295）；`_pick_app_client` 扩路由已 LIVE（`--skip-web`，api health OK）——H3：含 `RH_MinimaxHailuoH3*` + 任意 class_type 含 MiniMaxH3|MinimaxH3|HailuoH3 → `192.168.71.127:8195`（`TOIV_H3_BASE_URL`，已 redeploy）；QwenEdit→默认 `192.168.71.114:8194`（无显式 `TOIV_QWEN_EDIT_BASE_URL`）；设备验收：H3 `minimax_h3_*`+MiniMaxH3* OK；QwenEdit EditUtils OK、Advance 已 Y（`Comfyui-QwenEditUtils`）；能力扫描 OK v1 58→v8 163（+105）；下一波见 `.regen_tmp/capability_gap_scan_v8/actionable/next_wave_misc_packages.md`；通用池 workers 仍 `127:8196`/`116:8188`/`114:8193`；勿 stage `.regen_tmp`/产品 dirty/`rh_h3_presets.json`。补注：`RHMiniMaxH3*` 有公开包（`HM-RunningHub/ComfyUI_RH_MinMaxH3` 或 `RH-RunningHub/ComfyUI-RH-MiniMax-H3`，非 unlocalizable；仅装 H3 `:8195`）；QwenEdit Advance 缺→装 QwenEditUtils 于 `:8194`；装齐后扫 v9。进度：Advance 已 Y（`Comfyui-QwenEditUtils`@`:8194`）；H3 `:8195` 独占 `ComfyUI_RH_MinMaxH3`（`RHMiniMaxH3*` Y，通用池无）；通用 通用 P0 杂项已齐（见新 tip）；v9 已扫完。细化：`ComfyUI_RH_MinMaxH3`@`d6c5f7b0`→`RHMiniMaxH3*` 33 class Y；`RH_MinimaxHailuoH3*` 无同名→16 app soft-hide（公开 982→966）；通用 P0 杂项已齐（见新 tip）；v9 已扫完。status=`live_on_core_dirty`；STATE `capability_gap_soft_hide_dedicated_pool_2026_09_08`；updated_at 2026-09-08T21:50:00+08:00。

**2026-09-08 LIVE：通用池装 SoundFlow/LG_Tools/art-venture（设备管家）**：已装 SoundFlow + LG_Tools + art-venture；三 `class_type` 四端齐。status=`live_device_fyi`；STATE `general_pool_soundflow_lg_artventure_2026_09_08`；updated_at 2026-09-08T20:17:00+08:00。
...
**2026-09-08 LIVE：MODEL_SOURCES capability_gap_v3_v4 后计数更新**：`docs/MODEL_SOURCES.md` + `docs/MODEL_SOURCES.json`；**ok 411 / blocked 376 / total 787**（前 381/384/765）；updated `2026-09-08T18:17:37+08:00`；根目录无 `MODEL_SOURCES*`；后续每批追加。status=`inventory_updated_capability_gap_v3_v4`；STATE `model_sources_inventory_2026_09_08`；updated_at 2026-09-08T18:18:00+08:00。能力缺口 v3/v4；true gaps 仍 0。由 ToIV 模型下载追加；本 commit 含清单正文 + 五件套进度。
...
**2026-09-08 LIVE：MODEL_SOURCES capability_gap_batch1 后计数更新**：`docs/MODEL_SOURCES.md` + `docs/MODEL_SOURCES.json`；**ok 381 / blocked 384 / total 765**（前 370/383/753）；updated `2026-09-08T16:55:00+08:00`；根目录无 `MODEL_SOURCES*`；后续每批追加。status=~~`inventory_updated_capability_gap_batch1`~~ → `inventory_updated_capability_gap_v3_v4`（见上条）；STATE `model_sources_inventory_2026_09_08`；updated_at 2026-09-08T17:00:00+08:00。能力缺口 batch1；true gaps 仍 0；可本地化缺口清零口径仍有效。由 ToIV 模型下载追加；本 commit 含清单正文 + 五件套进度。
...
**2026-09-08 LIVE：MODEL_SOURCES qwen_edit_pc01_sync 后计数更新**：`docs/MODEL_SOURCES.md` + `docs/MODEL_SOURCES.json`；**ok 370 / blocked 383 / total 753**（前 368/383/751）；updated `2026-09-08T16:25:00+08:00`；根目录无 `MODEL_SOURCES*`；后续每批追加。status=~~`inventory_updated_qwen_edit_pc01`~~ → `inventory_updated_capability_gap_batch1`（见上条）；STATE `model_sources_inventory_2026_09_08`；updated_at 2026-09-08T16:27:00+08:00。增量=Qwen Edit pc01 同步（qwen_2.5_vl_7b_fp8_scaled + qwen_image_vae）；可本地化缺口清零口径仍有效。由 ToIV 模型下载追加；本 commit 含清单正文 + 五件套进度。

**2026-09-08 LIVE：生图/视频 worker 节点装齐 + pc01 NAS 临时恢复（设备管家）**：已装 WanVideoWrapper + LayerStyle + ComfyLiterals(Int) + Easy-Use；**无法本地装** RHHiddenNodes；pc01 NAS UNC ~~临时 `net use` 恢复~~ → 见上条：须 `net use \\192.168.71.7\NAS` (share=`NAS`)；脚本已改；WS≈581。status=`live_device_fyi`；STATE `worker_custom_nodes_pc01_nas_2026_09_08`；updated_at 2026-09-08T13:51:00+08:00。细节矩阵在设备管家侧；本 tip 只记活口径。

**2026-09-08 LIVE：可本地化缺权重清零（设备管家确认）**：原扫缺 missing **737** → **0**；MODEL_SOURCES batch16 清尾 ok **368** / blocked **383** / total **751**；blocked=gated/无溯源/近名。细节以 `.regen_tmp/` 与 `NAS/toiv/comfyui-models` 为准。status=~~`inventory_updated_batch16_gap_cleared`~~ → `inventory_updated_qwen_edit_pc01`（见上条）；STATE `model_sources_inventory_2026_09_08`；updated_at 2026-09-08T07:13:00+08:00。via 设备管家；清单正文已在 `baf514f`。
...
**2026-09-08 LIVE：MODEL_SOURCES batch14 后计数更新（历史；进一步片→batch15）**：`docs/MODEL_SOURCES.md` + `docs/MODEL_SOURCES.json`；**ok 321 / blocked 264 / total 585**（前 292/159/451）；updated `2026-09-08T06:17:23+08:00`；根目录无 `MODEL_SOURCES*`；后续每批追加。status=~~`inventory_updated_batch14`~~ → 见上条 `inventory_updated_batch15`；STATE `model_sources_inventory_2026_09_08`；updated_at 2026-09-08T06:18:00+08:00。由 ToIV 模型下载追加；本 commit 含清单正文 + 五件套进度。

**2026-09-08 LIVE：MODEL_SOURCES batch13 后计数更新（历史；进一步片→batch14）**：`docs/MODEL_SOURCES.md` + `docs/MODEL_SOURCES.json`；**ok 292 / blocked 159 / total 451**（前 262/130/392）；updated `2026-09-08T05:46:49+08:00`；根目录无 `MODEL_SOURCES*`；后续每批追加。status=~~`inventory_updated_batch13`~~ → 见上条 `inventory_updated_batch14`；STATE `model_sources_inventory_2026_09_08`；updated_at 2026-09-08T05:47:00+08:00。由 ToIV 模型下载追加；本 commit 含清单正文 + 五件套进度。
