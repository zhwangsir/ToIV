...
**2026-09-08 LIVE：封面效果对齐完成（修35；剩18待真出图）**：rh-acc **全 RH CDN**；修 **35**（5 rh-acc mp4→still CDN + 30 builtin 上传 RH 图）；剩约 **18** 无 RH 映射 builtin 待真出图（禁 NSFW `/covers/generate` 渐变）；可见 apps≈**811**；rh-acc 期间 sibling reseed **706→763**；市场家族 `rh-h3` 克隆当前 **0**；家族 wipe/reseed 仍在 sibling（另报）。status=`done_18_pending_real_art`；STATE `cover_effect_align_2026_09_08`；报告 `.regen_tmp/cover-effect-align-20260908.md`（勿提交）；updated_at 2026-09-08T03:12:00+08:00。产品树 dirty 未 commit 产品码；远程未推。

**2026-09-08 LIVE：RH 表单封面保真（research→拍板执行中）**：rh-acc **706** 修 **168** / 已齐 **529** / skip code_901 **9** / errors **0**；封面 RH CDN **706/706**；LIST→select 修复；去 graph fallback 多余字段；封面改写仅 **2**。真正错位主因≈**1166** 家族模板克隆（`rh-h3`/`rh-minimax*`，无 webappId，本地海报封面）。**用户拍板**：wipe 家族模板克隆 → RH 准确图+原表单重入库；keep=`rh-acc + product builtins`；ToIV **执行中**（**滚动**：市场家族 rh-h3 克隆当前 **0**；reseed 仍在 sibling）。status=`decided_executing`；decision=`wipe_family_clones_accurate_reseed`；STATE `rh_form_cover_fidelity_2026_09_08`；报告 `.regen_tmp/rh-form-cover-fidelity-20260908.md`（勿提交）；updated_at 2026-09-08T03:12:00+08:00。产品树 dirty 未 commit 产品码；远程未推。
