...
**2026-09-10 admin soft-hide 可再上架 + v20 P0/animate2 unhide 候选（项目管家；ToIV 开发+能力缺口 tip；产品 dirty 待用户提交）**：产品改动 **未提交** — `apps/api/app/routes/apps.py`：admin `_visible` 可见目录 soft-hide 卡并可 PUT `is_public=true` 再上架（seed **不覆盖** `is_public`）；`apps/api/tests/test_apps.py`：`test_admin_can_unhide_soft_hidden_catalog_and_builtin`；原因=mass soft-hide 后 admin 也无法 GET/PUT 隐藏卡（404）。**待用户产品 commit**（本 tip 只 docs；apps.py / tests 仍 dirty **勿 stage**）。能力缺口 v20 P0 unhide 候选已交开发：通用 P0 **80** + `ok_if_dedicated_animate2` **2** （`wan-animate-2` + `rh-character-replacement-minimax-h3-7582cc8e`，图含 WanAnimate2ToVideo；应走 **`:8199`**）；可先 unhide 该 2 张 → 公开约 **508→510**。真源 `.regen_tmp/capability_gap_scan_v20/actionable/unhide_after_fill_p0.json` **勿 stage**。产品 unhide 仍等开发落地或用户拍板（admin 可见/再上架码部署后更稳）。勿 stage `.regen_tmp`/产品 dirty；无产品 SHA。status=`pending_product_commit`；STATE `admin_soft_hide_relist_wan_animate2_2026_09_10`；updated_at 2026-09-10T15:40:00+08:00。
