...
**2026-09-03 Phase 2 H3 fl2v/r2v（未提交未部署）**：Phase 2 H3 图已落本机（**未提交、未部署**）。新引擎 `h3-fl2v` / `h3-nsfw-fl2v` → POST `/api/h3/fl2v`（首尾帧，LoadImage 100/101 → 节点 104 `first_frame`/`last_frame`）；`h3-r2v` / `h3-nsfw-r2v` → POST `/api/h3/r2v`（`MiniMaxH3ReferenceToVideo`，最多 9 图 3 视频 3 音频）。SFW Ref2VA UNET：`minimax_h3_ref2va_pruned_int8_convrot.safetensors`。pytest `test_h3_studio` + `test_app_seed` + `test_engine_registry` 共 136 过。应用市场 r2v 目前只绑 `image1`（app-run 单叶子），生成页 `images.max=9`。H3 worker 若无 `ReferenceToVideo` 节点会 503。改动：`h3_video.py`、`h3_studio.py`、`h3.py`、`engine_registry.py`、`app_seed.py`、`engines.ts` 及对应测试。AGENTS.md / 设备清单未动。
