...
**2026-09-11 拍板 P0实测→P1说明卡→P3扩展Admin→P2说明书KG（项目管家；ToIV 开发 tip；无产品 SHA）**：用户确认全序 — **P0** 分级实测（开发已开工 **P0 L0**，`in_progress`）→ **P1** 应用说明卡（用法/作用）→ **P3** 万能管理台（扩展现有 Admin，非另起后台）→ **P2** 说明书知识图谱。**非 shipped/online**（产品实现未 commit）。旁证：AdminView=users/agents/audit only；最小 KG `GET /api/admin/knowledge-graph` ≈1295/608、无 howto；公开 ≈550（video359/image189），描述 p50≈40。产品 dirty 勿 stage；无产品 SHA。status=`roadmap_confirmed`（P0 L0=`in_progress`；其余=`planned`）；STATE `roadmap_p0_p1_p3_p2_2026_09_11`；updated_at 2026-09-11T00:37:00+08:00。
