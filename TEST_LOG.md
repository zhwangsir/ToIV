...
- 2026-09-07 provenance 剥离 + Admin knowledge-graph（本地未 commit；部署中；无产品 SHA；非现网；docs only）：本地已加 provenance 剥离 + Admin knowledge-graph API；普通用户去 RH:{id}/civitai_url，管理员保留；AGENTS 未动，上线后再改口径。勿写 LIVE on core。
