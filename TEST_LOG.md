...
- 2026-09-23 992313 空闲窗终测 FAIL；封面闸假忙已修（项目管家；ToIV 开发 tip；docs；产品 SHA `1292bb5` 已部署未合 main）：**FAIL** `rh-acc-9923136513-48997d`（Z-Image 去 AI 感）idle-window 最终 smoke / `runtime_cuda`——finegrained-fp8 `kernels` 版本冲突（现要求 `0.15.2≤v<0.16.0`，舰队 `0.16.2`；:8196 restart+0.16.2 trust_ok 复测仍 FAIL；设备管家在裁处）；测时 cover_gate 已解锁+队列空闲；封面闸假忙产品修 `1292bb5`（`sum_queue_depth(force=)`）已部署 core，生产 `TOIV_COVER_AUTOREFIRE=false`；轮询例程已删；产品分支未合 main；勿改 AGENTS 集群真源；勿 stage `.regen_tmp`；status=`idle_window_fail_runtime_cuda_kernels`；STATE `rh_acc_992313_fail_cover_gate_fix_2026_09_23`；updated_at 2026-09-23T05:49:00+08:00；AGENTS §七/§八 已短注。

- 2026-09-22 model_sources+app_test_matrix core 已 live；992313 封面闸暂缓（项目管家；ToIV 开发 tip；docs；无产品 SHA）：`model_sources.json`+`app_test_matrix/` **生产已齐**（真源 `/home/merlin/toiv/api/app/data/`；md5 `6aeded7b…` ok478/blocked336/total815；五文件 mtime 一致；无需再 scp/restart）；`rh-acc-9923136513` 终测 **暂缓**（cover_gate gated queue_depth 13>12，pending≈4461）；产品分支 `feat/app-guides-admin-cms` @ `b30806d` 超前 main `bab16b3` 先不推；勿 stage `.regen_tmp`；status=`already_live_verified`；STATE `model_sources_core_sync_2026_09_22`；updated_at 2026-09-22T21:05:00+08:00；AGENTS §七/§八 已短注/核销。

- 2026-09-11 466169 PASS；145828 SeedVR2 转设备（项目管家；ToIV 开发 tip；docs；无产品 SHA）：**PASS** `rh-acc-4661695490` LTX distilled-lora @ `:8196`；**FAIL→设备** `rh-acc-1458284545` SeedVR2 — Comfy 目录可见但仍 Failed to download（节点自有路径，已转设备）；前置 tip `3062f66`；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_466169_pass_145828_seedvr2_device`；STATE `public_e2e_retest_466169_145828_2026_09_11`；updated_at 2026-09-11T10:40:00+08:00；AGENTS §七 已短注。

- 2026-09-11 产品 dirty 续；221805/315353 PASS（项目管家；ToIV 开发 tip；docs；无产品 SHA）：产品 dirty（**未** commit）续 — KSampler `beta57`→`beta`、VHS `skip_first_frames` 等；**PASS** `rh-acc-2218056705`、`rh-acc-3153534978`；仍跟 `426919` running；LayerMask 需 worker `models/segformer_b3_clothes` / `segformer_b2_clothes`；Qwen3_VQA 需 `LLM/Qwen3-VL-4B-Instruct`→`models/prompt_generator/`；前置 tip `a1db34c`；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_dirty_product_beta57_vhs_pass2`；STATE `public_e2e_retest_product_dirty_beta57_vhs_2026_09_11`；updated_at 2026-09-11T10:28:00+08:00；AGENTS §七 已短注。

- 2026-09-11 产品 dirty 回填；144483 PASS（项目管家；ToIV 开发 tip；docs；无产品 SHA）：产品 dirty（**未** commit）— easy promptLine `remove_empty_lines`、INPAINT `blur_type`/`device_mode`、Wan VACE `num_frames`、丢弃无 images 孤儿 VHS；**PASS** `rh-acc-1444836353` @ `:8197`；`294829` 字段已清 queued_timeout @ `:8194` → 重测；`142022` 字段已清 → 缺 darkBeastMar0326 / flux2-vae / qwen_3_8b + Flux2 节点（转下载/设备）；前置 tip `db38be3`；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_dirty_product_backfill_144483_pass`；STATE `public_e2e_retest_product_dirty_backfill_2026_09_11`；updated_at 2026-09-11T10:26:00+08:00；AGENTS §七 已短注。

- 2026-09-11 451079 PASS；387721 新缺 SeCVideoSegmentation（项目管家；ToIV 开发 tip；docs；无产品 SHA）：**PASS** `rh-acc-4510792705`（VACE HIGH 后）@ `:8197`；**FAIL→设备** `rh-acc-3877213185` — PDIMAGE 已清，新缺 `SeCVideoSegmentation`（已转设备）；前置 tip `4f89e8b`；本地 stack `8f33a2c`；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_451079_pass_387721_sec_device`；STATE `public_e2e_retest_451079_387721_2026_09_11`；updated_at 2026-09-11T10:24:00+08:00；AGENTS §七 已短注。

- 2026-09-11 公开 E2E wave4；36 卡 PASS6/FAIL26/TIMEOUT4（项目管家；ToIV 开发 tip；docs；无产品 SHA）：公开 E2E **wave4** 先前未测 **36** 卡 — **PASS 6** / **FAIL 26** / **TIMEOUT 4**；累计约 **~144 tested / 550**（波次 PASS ~59）；残差 — **可重试** 熔断/503/上传若干；**设备** Anything Everywhere3 / SAM3 Get Object Mask / 全局输入 / H3 文本节点 / IPAdapter FaceID LoRA；**产品候选** LayerMask abs-path / beta57 / Qwen3_VQA HF / LoadImage toivref；真源 `public_e2e_wave4.json`（**勿 stage**）；前置 tip `4f89e8b`；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_wave4_done_triaged`；STATE `public_e2e_wave4_2026_09_11`；updated_at 2026-09-11T10:23:00+08:00；AGENTS §七 已短注。

- 2026-09-11 wave3 设备复测；PASS1/FAIL5（项目管家；ToIV 开发 tip；docs；无产品 SHA）：wave3 **设备复测** — **PASS 1** / **FAIL 5**；PASS `rh-acc-1126448130` @ `:8197`；仍缺设备 `PDIMAGE:LongerSize`(387721) / `VACE/…_HIGH_bf16` 前缀路径(451079)；交产品默认回填 `easy promptLine.remove_empty_lines`(294829) / `INPAINT_ExpandMask.blur_type`(142022)；另查 144483 VHS 缺 images/num_frames；387721 等 PDIMAGE；前置 tip `7865dd0`；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_wave3_device_pass1_fail5`；STATE `public_e2e_retest_wave3_device_2026_09_11`；updated_at 2026-09-11T09:58:00+08:00；AGENTS §七 已短注。

- 2026-09-11 公开 E2E wave3；36 卡 PASS19/FAIL17（项目管家；ToIV 开发 tip；docs；无产品 SHA）：公开 E2E **wave3** 先前未测 **36** 卡 — **PASS 19** / **FAIL 17** / **TIMEOUT 0**；累计约 **~120 tested / 550**；失败分流 — **设备** Primitive integer [Crystools]×3 / easy promptLine.remove_empty_lines / INPAINT_ExpandMask.blur_type / VACE weights invisible；**模型** SeedVR2 DiT+VAE / LTX distilled-lora 384 / scheduler beta57；**产品** HF ConnectTimeout/no snapshot / LayerMask abs→HF repo_id×2 / Preview-only×3 / VHS/MimicMotion empty；真源 `public_e2e_wave3.json`（**勿 stage**）；前置 tip `8139067`；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_wave3_done_triaged`；STATE `public_e2e_wave3_2026_09_11`；updated_at 2026-09-11T09:25:00+08:00；AGENTS §七 已短注。

- 2026-09-11 214347 PASS；wave2 nodes3 FAIL 均清（项目管家；ToIV 开发 tip；docs；无产品 SHA）：`rh-acc-2143478786` **PASS**（设备 DrawViTPose `crop_y0` 修后）@ `:8197` job=`88a6c2b6…`；media 两段 Wanimate audio mp4；wave2 nodes3 两 FAIL 均清（417975 已 PASS；214347 现 PASS）；前置 tip `72d158e`；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_214347_pass_nodes3_fails_cleared`；STATE `public_e2e_retest_214347_pass_2026_09_11`；updated_at 2026-09-11T08:47:00+08:00；AGENTS §七 已短注。

- 2026-09-11 wave2 nodes3 残差；417975 PASS；214347 转设备（项目管家；ToIV 开发 tip；docs；无产品 SHA）：**PASS** `rh-acc-4179756034` `_normalize_easy_image_rembg` rem_mode RMBG-2.0→1.4 @ `:8197` job=`ad581aea…`；**FAIL→设备** `rh-acc-2143478786` WanVideoModelLoader remap 后 DrawViTPose `crop_y0` UnboundLocalError（已转设备管家）；dirty `apps.py` `_normalize_wan_video_model_loader`+`_normalize_easy_image_rembg` 勿 stage；前置 tip `d43956d`；非 shipped/online；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_dirty_wave2_nodes3_417975_pass_214347_device`；STATE `public_e2e_retest_wave2_nodes3_fix_2026_09_11`；updated_at 2026-09-11T08:38:00+08:00；AGENTS §七 已短注。

- 2026-09-11 wave2 nodes3；PASS1/FAIL2（项目管家；ToIV 开发 tip；docs；无产品 SHA）：after RMBG+HF — **PASS 1 / FAIL 2**；PASS `rh-acc-2137763841` MimicPose+VACE mp4×2 @ `:8197`；FAIL `rh-acc-2143478786` RMBG 已有仍无 media（仅 MathExpression/showAnything；WanAnimate/VHS 未产出）→ 产品跟；FAIL `rh-acc-4179756034` HF timeout 已清，残差 `'Config' object has no attribute 'model_type'`（easy imageRemBg）→ 产品/节点查；前置 tip `0779810`；相关 wave2 nodes2；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_wave2_nodes3_pass1_fail2`；STATE `public_e2e_retest_wave2_nodes3_2026_09_11`；updated_at 2026-09-11T08:23:00+08:00；AGENTS §七 已短注。

- 2026-09-11 107403 PASS；三残差均清（项目管家；ToIV 开发 tip；docs；无产品 SHA）：`rh-acc-1074032641` **PASS** @ `:8197`；job=`551f095d9b5b4ad4813d374d43b34c0a`；prompt=`d388d73a…`；media `视频/Name_WAN-VACE_00001.gif` + `00002.gif`；wall ~6063s（mostly queued after WanAnimate `f10ef2f0`）；先验残差 `059347`/`022665`/`107403` 均 **PASS**；前置 tip `6310634`；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_107403_pass_trio_cleared`；STATE `public_e2e_retest_107403_pass_2026_09_11`；updated_at 2026-09-11T08:12:00+08:00；AGENTS §七 已短注。

- 2026-09-11 022665 强制 :8196 再 PASS；SDPA（项目管家；ToIV 开发 tip；docs；无产品 SHA）：`rh-acc-0226654210` 强制 `:8196` **再 PASS**（设备 QwenVL sm120 sageattn→SDPA）；job=`f7fbd951…`；log `Auto mode: Using SDPA` + FP16 override；此前 PASS @`:8193`；前置 tip `19ce070`；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_022665_8196_sdpa_pass`；STATE `public_e2e_retest_022665_8196_sdpa_2026_09_11`；updated_at 2026-09-11T07:58:00+08:00；AGENTS §七 已短注。

- 2026-09-11 059347/022665 PASS；107403 仍排队（项目管家；ToIV 开发 tip；docs；无产品 SHA）：`rh-acc-0593472514` **PASS** @ `:8193`（设备 `AILab_QwenVL` sm120+ Q8→FP16）；`rh-acc-0226654210` **PASS** @ `:8193`（`_normalize_ltxv_dynamiccombo` bare `image_1`→`num_guides.*` + LatentUpscale `x2-1.0`→`1.1`）；仍排队 `rh-acc-1074032641` @ `:8197` after WanAnimate；前置 tip `7b32907`；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_dirty_059347_022665_pass_107403_queued`；STATE `public_e2e_retest_059347_022665_pass_2026_09_11`；updated_at 2026-09-11T07:38:00+08:00；AGENTS §七 已短注。

- 2026-09-11 QwenVL/LTXV/Wan dirty；三卡 PASS；三残差（项目管家；ToIV 开发 tip；docs；无产品 SHA）：产品 dirty（已部署、未 commit）— `AILab_QwenVL` `video_frame_size` / `LTXV` DYNAMICCOMBO `image_*` / Wan `teacache`·`experimental`·VAE names；PASS `052027`/`293570`/`205312`；残差 `022665` shape、`059347` cublasLt、`107403` 排队；真源 `public_e2e_retest_qwen3vl.json` / `ltx_dynamiccombo` / `public_e2e_retest_107403.json`（**勿 stage**）；前置 tip `fbd9ac3`；相关 `5e2fe3d`/`2dfb295`/wave2 107403；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_dirty_qwenvl_ltx_wan_partial_pass`；STATE `public_e2e_qwenvl_ltx_wan_dirty_retest_2026_09_11`；updated_at 2026-09-11T07:09:00+08:00；AGENTS §七 已短注。

- 2026-09-11 011723 JoyCaption 三机镜像 PASS@8196（项目管家；ToIV 开发 tip；docs；无产品 SHA）：`011723` JoyCaption 三机镜像后 **PASS** @ `:8196`；清 tip `b3456e2` 残差（JoyCaption HF 仅在 `:8193`、池子派 `:8196`；已转设备镜像）；真源 `public_e2e_retest_011723_joycaption.json`（**勿 stage**）；前置 tip `b3456e2` wave1 设备复测（本地 parent）；相关 `public_e2e_retest_wave1_device_2026_09_11` 残差已清；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_011723_joycaption_pass`；STATE `public_e2e_retest_011723_joycaption_2026_09_11`；updated_at 2026-09-11T06:43:00+08:00；AGENTS §七 已短注。

- 2026-09-11 wave1 设备复测；pulid/removebg PASS；JoyCaption 8193（项目管家；ToIV 开发 tip；docs；无产品 SHA）：`pulid`/`removebg` **PASS**；`011723` JoyCaption HF 仅在 `:8193`，池子却派到 `:8196`；**已转设备镜像**；真源 `public_e2e_retest_wave1_device.json`（**勿 stage**）；前置 tip `6c72c16` wave2 nodes2（本地 parent）；相关 `public_e2e_wave1_product_leftovers_2026_09_11`；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_wave1_device_handed_mirror`；STATE `public_e2e_retest_wave1_device_2026_09_11`；updated_at 2026-09-11T06:16:00+08:00；AGENTS §七 已短注。

- 2026-09-11 wave2 nodes2；三旧清；RMBG/HF timeout（项目管家；ToIV 开发 tip；docs；无产品 SHA）：三旧节点清（`MimicMotionGetPoses` / `DF_Int_to_Float` / `Florence2Run`）；`214347` 缺 **RMBG**；`213776`/`417975` **HF ConnectTimeout**；**已转设备**；真源 `public_e2e_retest_wave2_nodes2.json`（**勿 stage**）；前置 tip `8b2eb2e` wave2 节点复测（本地 parent）；相关 `public_e2e_retest_wave2_nodes_2026_09_11`；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_wave2_nodes2_handed_device`；STATE `public_e2e_retest_wave2_nodes2_2026_09_11`；updated_at 2026-09-11T06:12:00+08:00；AGENTS §七 已短注。

- 2026-09-11 wave2 节点复测；038828 PASS；新缺三节点（项目管家；ToIV 开发 tip；docs；无产品 SHA）：`038828` **PASS**；`213776`/`214347`/`417975` 新缺 `MimicMotionGetPoses`、`DF_Int_to_Float`、`Florence2Run`；`107403` **Preview-only**；真源 `public_e2e_retest_wave2_nodes.json`（**勿 stage**）；前置 tip `2dfb295` LTX23 复测（本地 parent）；相关 `public_e2e_wave2_2026_09_11`；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_wave2_nodes_retest`；STATE `public_e2e_retest_wave2_nodes_2026_09_11`；updated_at 2026-09-11T05:58:00+08:00；AGENTS §七 已短注。

- 2026-09-11 LTX23 复测；缺模清；image_1 残差（项目管家；ToIV 开发 tip；docs；无产品 SHA）：app `2053122050` **缺模清**；新残差 `LTXVImgToVideoInplaceKJ` unexpected `image_1`；ToIV 开发 **正修**；真源 `public_e2e_retest_ltx23.json`（**勿 stage**）；前置 tip `5e2fe3d` Qwen3-VL 复测（本地 parent）；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_ltx23_retest_fixing`；STATE `public_e2e_retest_ltx23_2026_09_11`；updated_at 2026-09-11T05:47:00+08:00；AGENTS §七 已短注。

- 2026-09-11 Qwen3-VL 复测；缺模清；022665/Preview-only（项目管家；ToIV 开发 tip；docs；无产品 SHA）：缺模清 `Florence2ModelLoader`+`LLM` 进 `model_names`；`022665` 缺 `video_frame_size`；`059347`/`052027`/`293570` **Preview-only**；ToIV 开发 **正修**；真源 `public_e2e_retest_qwen3vl.json`（**勿 stage**）；前置 tip `4aade9b` wave1 产品残差（本地 parent）；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_qwen3vl_retest_fixing`；STATE `public_e2e_retest_qwen3vl_2026_09_11`；updated_at 2026-09-11T05:44:00+08:00；AGENTS §七 已短注。

- 2026-09-11 wave1 产品残差；775482/nunchaku PASS；JoyCaption/pulid（项目管家；ToIV 开发 tip；docs；无产品 SHA）：`775482` **PASS**（Float→Number）；`flux1-nunchaku` **PASS** @ `:8188`；`011723` 清 `resolution_steps` 后卡 **JoyCaption HF**；`pulid`/`removebg` → 交设备；真源 `WAVE1_PRODUCT_LEFTOVERS.md`（**勿 stage**）；前置 tip `a78a8d3` MODEL_SOURCES wave2（本地 parent）；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_dirty_wave1_product_leftovers`；STATE `public_e2e_wave1_product_leftovers_2026_09_11`；updated_at 2026-09-11T05:42:00+08:00；AGENTS §七 已短注。

- 2026-09-11 MODEL_SOURCES wave2；ok468/blocked335/total803（项目管家；ToIV 模型下载 tip；docs；无产品 SHA）：wave2 inventory **ok 468 / blocked 335 / total 803**；摘要 `public_e2e_wave2_download.json`（**勿 stage**）；前置 tip `0451863` MelBand 039434 PASS（本地 parent）；相关公开 E2E wave2 tip `9bda626`；wave1 记忆 ok≈465/total≈800 → 本条 wave2 正式计数；相对 v16 461/343/804 Δ +7/−8/−1；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`inventory_updated`；STATE `model_sources_wave2_public_e2e_2026_09_11`；updated_at 2026-09-11T05:28:00+08:00；AGENTS §七 已短注。

- 2026-09-11 MelBand 039434 PASS；normalize+sampler dirty（项目管家；ToIV 开发 tip；docs；无产品 SHA）：MelBand `039434` **PASS**；dirty 部署（**未** commit）— `_normalize_melband_roformer` FS path→registry display name；sampler 默认值回填；真源 `public_e2e_retest_melband_save_preview.json`（**勿 stage**）；清 tip `b1a8cc7` / STATE `public_e2e_retest_melband_quant_2026_09_11` Preview-only 残差；前置 tip `9bda626` 公开 E2E wave2（本地 parent）；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_dirty_melband_039434_pass`；STATE `public_e2e_retest_melband_pass_2026_09_11`；updated_at 2026-09-11T05:26:00+08:00；AGENTS §七 已短注。

- 2026-09-11 公开 E2E wave2；36 卡 PASS11/FAIL22/TIMEOUT3（项目管家；ToIV 开发 tip；docs；无产品 SHA）：公开 E2E **wave2** **36** 卡 — **PASS 11** / **FAIL 22** / **TIMEOUT 3**；累计约 **46/550**；真源 `public_e2e_wave2.json`（**勿 stage**）；失败分流 — **设备节点** ×6；**权重** Qwen3-VL/LTX23；**产品侧** 上传 502 / worker 丢作业 / Preview-only；前置 tip `b1a8cc7` 109907 PASS；MelBand Preview-only 残差（本地 parent）；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_wave2_done_triaged`；STATE `public_e2e_wave2_2026_09_11`；updated_at 2026-09-11T05:20:00+08:00；AGENTS §七 已短注。

- 2026-09-11 109907 PASS；MelBand 039434 Preview-only 残差（项目管家；ToIV 开发 tip；docs；无产品 SHA）：复测 `109907` **PASS**；MelBand `039434` 清 model_names 后残差 = **Preview-only / 主保存未产出**；真源 `public_e2e_retest_melband_quant.json`（**勿 stage**）；ToIV 开发 **正查产品**；前置 tip `8451b54` MelBand+WanVideo dirty 再部署；复测 039434/109907（本地 parent）；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_109907_pass_melband_preview_residual`；STATE `public_e2e_retest_melband_quant_2026_09_11`；updated_at 2026-09-11T05:02:00+08:00；AGENTS §七 已短注。

- 2026-09-11 MelBand+WanVideo dirty 再部署；复测 039434/109907（项目管家；ToIV 开发 tip；docs；无产品 SHA）：产品 **dirty re-deployed** 到 core（**仍未** git commit）— `MelBandRoFormerModelLoader` 入 `model_names` + scan `vocal_separator`/`MelBandRoFormer`；WanVideo 非 scaled 权重 strip `_scaled` quantization；复测中 `039434` / `109907`；前置 tip `93d74fa` H3 r2v 三卡 PASS；109907 quant `_scaled` 残差（本地 parent）；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_dirty_melband_wan_quant_retesting`；STATE `public_e2e_melband_wan_quant_dirty_redeploy_2026_09_11`；updated_at 2026-09-11T04:51:00+08:00；AGENTS §七 已短注。

- 2026-09-11 H3 r2v 三卡 PASS；109907 quantization _scaled 残差（项目管家；ToIV 开发 tip；docs；无产品 SHA）：H3 三卡 **PASS**（`h3-r2v` / `h3-r2v-voice` / `rh-2k-h3`）— AUTOGROW 清；`109907` 新残差：WanVideoModelLoader quantization `_scaled` 与非 scaled 权重冲突；真源 `public_e2e_retest_h3_109907.json`（**勿 stage**）；正改；前置 tip `2a864ee` H3 AUTOGROW dirty 部署复测中（本地 parent）；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_h3_pass_109907_quant_fix`；STATE `public_e2e_retest_h3_109907_2026_09_11`；updated_at 2026-09-11T04:45:00+08:00；AGENTS §七 已短注。

- 2026-09-11 H3 AUTOGROW + Text Multiline dirty 部署；复测中（项目管家；ToIV 开发 tip；docs；无产品 SHA）：产品 **dirty deployed** 到 core（**未** git commit）— H3 **AUTOGROW** ref mapping + Text Multiline **dynamic_prompts** backfill；复测中：`h3-r2v` / `h3-r2v-voice` / `rh-2k-h3` + `109907`；前置 tip `6856f2c` MelBand 复测 FAIL / model_names 正改（本地 parent）；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_dirty_h3_autogrow_dynamic_prompts_retesting`；STATE `h3_autogrow_dynamic_prompts_dirty_deploy_2026_09_11`；updated_at 2026-09-11T04:42:00+08:00；AGENTS §七 已短注。

- 2026-09-11 MelBand 复测 FAIL；model_names 漏 MelBandRoFormerModelLoader；正改 client.py（项目管家；ToIV 开发 tip；docs；无产品 SHA）：MelBand 复测仍 **FAIL**；根因：FS 已有权重，但 core `model_names` **未含** `MelBandRoFormerModelLoader` → pool 误判缺模型；真源 `public_e2e_retest_melband.json`（**勿 stage**）；正改 `client.py` `model_names`（产品 dirty，未 commit）；前置 tip `39a536d`（dual-pushed）wave1 节点复测；MelBandRoformer 曾在 wave1 下载分流；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_melband_model_names_fix`；STATE `public_e2e_retest_melband_2026_09_11`；updated_at 2026-09-11T04:37:00+08:00；AGENTS §七 已短注。

- 2026-09-11 wave1 节点复测；237247 PASS；109907 残差 VAE/Text Multiline（项目管家；ToIV 开发 tip；docs；无产品 SHA）：公开 E2E wave1 **节点复测** — `237247` **PASS**；`109907` 清节点后残差：`Wan2.1_VAE.pth` 名/格式 + Text Multiline `dynamic_prompts`；真源 `public_e2e_wave1_retest_nodes.json`（**勿 stage**）；前置 tip `266b988`（dual-pushed）wave1 36 PASS23/FAIL13；正修 H3；设备分流含 Text Multiline；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_wave1_retest_nodes`；STATE `public_e2e_wave1_retest_nodes_2026_09_11`；updated_at 2026-09-11T04:35:00+08:00；AGENTS §七 已短注。

- 2026-09-11 公开 E2E wave1；36 卡 PASS23/FAIL13；正修 H3（项目管家；ToIV 开发 tip；docs；无产品 SHA）：公开 E2E **wave1** **36** 卡 — **PASS 23** / **FAIL 13** / **TIMEOUT 0**；真源 `public_e2e_wave1.json`（**勿 stage**）；失败分流 — **产品**：H3 `ref_image_1` ×3（及同类）；**设备**：CR Prompt Text、Load Multiline；**下载**：Qwen3-VL、MelBandRoformer；当前正修 **H3**；前置 tip `092a7f8`（dual-pushed）inventory 550；L2 12/12/526；wave1 ~20 started（现扩至 36）；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_wave1_done_fixing_h3`；STATE `public_e2e_wave1_2026_09_11`；updated_at 2026-09-11T04:26:00+08:00；AGENTS §七 已短注。

- 2026-09-11 公开 E2E inventory；550；L2 12/12/526；第一波~20 已开（项目管家；ToIV 开发 tip；docs；无产品 SHA）：公开 E2E inventory **550** 公开 apps；L2 迄今 **PASS 12** / **FAIL 12** / **untested 526**；真源 `public_e2e_inventory.json` / `PUBLIC_E2E_PROGRAM.md`（**勿 stage**）；第一波 ~**20** live runs **已开**；前置 tip `bdead05`（dual-pushed）收口定义锁定；公开全量真跑矩阵开工；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`live_on_core_public_e2e_inventory_wave1`；STATE `public_e2e_inventory_2026_09_11`；updated_at 2026-09-11T03:43:00+08:00；AGENTS §七 已短注。

- 2026-09-11 收口定义锁定；公开全量真跑矩阵开工（项目管家；ToIV 开发 · 用户锁定；docs；无产品 SHA）：**收口定义**：每个**公开市场** app 须 **submit → 产出视频/图片 PASS** 才算可用（「都能用」）；软隐藏 **hard-blocked** apps **不入**下一步规划基线；开工 **公开全量真跑矩阵**：inventory → batched E2E → 失败分流 产品 / 设备 / 下载；前置 tip `2ac5a3b`（dual-pushed）116926/140266 PASS；CUDA 残差清；LongCat WanAnimate 热路径两卡收口；本规划/矩阵开工本身非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`roadmap_public_e2e_matrix_kickoff`；STATE `public_e2e_pass_closeout_def_2026_09_11`；updated_at 2026-09-11T03:38:00+08:00；AGENTS §七 已短注。

- 2026-09-11 sageattn SDPA 后复测；116926/140266 PASS；CUDA 残差清（项目管家；ToIV 开发 tip；docs；无产品 SHA）：sageattn@sm120 **SDPA fallback** 后复测 — `116926` / `140266` 均 **PASS**；**WanVideoSampler CUDA 残差已清**；LongCat WanAnimate 热路径两卡 **收口**；真源 `.regen_tmp/app_test_matrix_p0/l2_retest_after_sageattn_sdpa.json`（**勿 stage**）；前置 tip `5349000` CUDA 假报=sageattn@sm120；SDPA 回退；重启 :8197；复测中 → **现已 PASS**；非 shipped/online；产品 dirty 勿 stage；无产品 SHA；本地 tip 无 push；status=`live_on_core_l2_sageattn_sdpa_pass_closeout`；STATE `l2_retest_after_sageattn_sdpa_2026_09_11`；updated_at 2026-09-11T03:22:00+08:00；AGENTS §七 已短注。

- 2026-09-11 LongCat CUDA 假报=sageattention@sm120；SDPA 回退重启 :8197；复测中（项目管家；ToIV 开发 tip；docs；无产品 SHA）：WanVideoSampler `Tensor must be on CUDA` **假报**根因 — **sageattention @ sm120**；已应用 **SDPA fallback patch** 并 **重启 LongCat `:8197`**；`116926` / `140266` 复测 **进行中**；真源 `.regen_tmp/app_test_matrix_p0/l2_retest_after_sageattn_sdpa.json` **待**（expected path，可能尚不存在；**勿 stage**）；前置 tip `89cd349` HF DWPose/SAM2 已清、残差 WanVideoSampler CUDA @ LongCat；非 shipped/online；产品 dirty 勿 stage；无产品 SHA；本地 tip 无 push；status=`live_on_core_l2_sageattn_sdpa_fallback_retesting`；STATE `l2_sageattn_sdpa_fallback_2026_09_11`；updated_at 2026-09-11T03:13:00+08:00；AGENTS §七 已短注。

- 2026-09-11 HF DWPose/SAM2 已清；新残差 WanVideoSampler CUDA @ LongCat（项目管家；ToIV 开发 tip；docs；无产品 SHA）：HF DWPose/SAM2 后 L2 复测 — **已清**：HF **DWPose** / **SAM2**；**新残差**：两张 RH 卡共用 **WanVideoSampler** 错误 — `Tensor must be on CUDA`（**LongCat**）；真源 `.regen_tmp/app_test_matrix_p0/l2_retest_after_hf_dwpose_sam2.json` **勿 stage**；已交设备管家；前置 tip `9eaeffc` DWPose HF 无 local / SAM2 HF timeout；Int/ONNX/Wan 已清；非 shipped/online；产品 dirty 勿 stage；无产品 SHA；本地 tip 无 push；status=`live_on_core_l2_retest_after_hf_dwpose_sam2`；STATE `l2_retest_after_hf_dwpose_sam2_2026_09_11`；updated_at 2026-09-11T03:09:00+08:00；AGENTS §七 已短注。

- 2026-09-11 ONNX/Wan 后复测；DWPose/SAM2 残差；Int/ONNX/Wan 已清（项目管家；ToIV 开发 tip；docs；无产品 SHA）：ONNX/Wan22path 后 L2 复测 — 新残差：`116926` **DWPose**（HF，无 local）；`140266` **SAM2**（HF timeout）；**已清**：Int / ONNX / Wan；真源 `.regen_tmp/app_test_matrix_p0/l2_retest_after_onnx_wan22path.json` **勿 stage**；已交设备管家预缓存；前置 tip `e0706fc` Int/Preview dirty 部署上 core；复测进行中；非 shipped/online；产品 dirty 勿 stage；无产品 SHA；本地 tip 无 push；status=`live_on_core_l2_retest_after_onnx_wan22path`；STATE `l2_retest_after_onnx_wan22path_2026_09_11`；updated_at 2026-09-11T02:53:00+08:00；AGENTS §七 已短注。

- 2026-09-11 Int/Preview 修复 dirty 部署上 core；复测 140266/116926 中（项目管家；ToIV 开发 tip；docs；无产品 SHA）：Int Number normalization + Preview-only **does not** mark_done 已经 `deploy.sh --skip-web` 上 core；**未** git commit（dirty deploy only）；`apps.py` / `tracker.py` sha 已核（真源 `.regen_tmp/app_test_matrix_p0/deploy_int_tracker.md`：apps=`21f1acf7…` tracker=`ae3fc155…`；**勿 stage**）；`140266` / `116926` 复测 **进行中**；前置 tip `1e94140` dirty deploy 现已落地；非 shipped/online；产品 dirty 勿 stage；无产品 SHA；本地 tip 无 push；status=`live_on_core_dirty_int_preview_fix_retesting`；STATE `l2_int_preview_dirty_deploy_2026_09_11`；updated_at 2026-09-11T02:31:00+08:00；AGENTS §七 已短注。

- 2026-09-11 L2 RH 残差根因；Int→Number；Preview 误 mark_done；将 dirty deploy（项目管家；ToIV 开发 tip；docs；无产品 SHA）：残差根因 — ComfyLiterals **Int** 必须 **`Number`**（校验）；**Preview-only** 误 **`mark_done`**；产品改动已落地但未 git commit，将 dirty deploy；`140266` 仍缺 `yolov10m` / `vitpose` + 权重 **`Wan22Animate/`** 前缀；真源 `.regen_tmp/app_test_matrix_p0/l2_rh_residual_notes.md` **勿 stage**；前置 tip `1eaa34e`：002481 PASS；140266 无产物；116926 Int Number + Wan2_2-Animate-14B_fp8…KJ 缺；非 shipped/online；产品 dirty 勿 stage；无产品 SHA；本地 tip 无 push；status=`live_on_core_l2_rh_residual_rootcause_pending_dirty_deploy`；STATE `l2_rh_residual_rootcause_2026_09_11`；updated_at 2026-09-11T02:29:00+08:00；AGENTS §七 已短注。

- 2026-09-11 10-class 后复测；002481 PASS；140266 无产物；116926 仍缺（项目管家；ToIV 开发 tip；docs；无产品 SHA）：10-class 安装后 L2 复测 — `002481`（`rh-acc-0024814594-adc75c`）**PASS**；`140266`（`rh-acc-1402669058-f27871`）job 成功但 **无产物/无输出**；`116926`（`rh-acc-1169267714-43e935`）仍缺 **Int Number** 校验 + 缺权重 `Wan2_2-Animate-14B_fp8_e4m3fn_scaled_KJ.safetensors`；真源 `.regen_tmp/app_test_matrix_p0/l2_retest_after_10class.json` **勿 stage**；已交模型下载/设备管家；非 shipped/online；产品 dirty 勿 stage；无产品 SHA；本地 tip 无 push；status=`live_on_core_l2_retest_after_10class`；STATE `l2_retest_after_10class_2026_09_11`；updated_at 2026-09-11T02:19:00+08:00；AGENTS §七 已短注。
...
- 2026-09-11 LongCat :8197 链入 Bjornulf/WanAnimatePreprocess/controlnet_aux；三目标 class Y（项目管家；docs；via 设备管家）：LongCat `:8197` 已链入 `Bjornulf_custom_nodes` / `WanAnimatePreprocess` / `controlnet_aux`；三目标 class **Y**（Bjornulf_ShowFloat / PoseAndFaceDetection / PixelPerfectResolution）；关 L2 dirty-deploy 残差节点缺口；wan-animate ffmpeg/fixture 仍开；产品 dirty 勿 stage；无产品 SHA；本地 tip 无 push；status=`live_device_fyi`；STATE `longcat_8197_nodes_bjornulf_wananim_ctrlaux_2026_09_11`；updated_at 2026-09-11T01:33:00+08:00；AGENTS §七 已短注。

- 2026-09-11 L2 路由 dirty 部署；抽测4/4 improved、pass0（项目管家；ToIV 开发 tip；docs；无产品 SHA）：live_core `deploy.sh --skip-web`（~01:27 CST）dirty 部署 L2 路由修复；**产品未 git commit**；抽测 **4/4** `routing_improved=true`、**pass=0**；残差：缺节点 3 class_type（Bjornulf_ShowFloat / PoseAndFaceDetection / PixelPerfectResolution）+ wan-animate ffmpeg/fixture；真源 `.regen_tmp/app_test_matrix_p0/l2_retest_after_routing.json` **勿 stage**；已交设备管家装缺节点；非 shipped/online（仅 dirty-deployed）；产品 dirty 勿 stage；无产品 SHA；本地 tip 无 push；status=`live_on_core_dirty_routing_improved_pass0`；STATE `l2_routing_dirty_deploy_retest_2026_09_11`；updated_at 2026-09-11T01:30:00+08:00；AGENTS §七 已短注。

- 2026-09-11 L2 路由修复（分支未commit；pytest52；抽测中）（项目管家；ToIV 开发 tip；docs；无产品 SHA）：**L2 路由修复**工作树已落地、**尚未 git commit**（`feat/app-guides-admin-cms` uncommitted）；`_pick_app_client` 识别 **WanVideo*** → 专池；`/run` + upload media 与 graph **同机**；pytest routing/upload/apps：**52 passed**；正部署抽测原 **503** 卡；残差：LongCat 缺权重；nunchaku 溢出；pulid EOF；无 LTX 专池；下一步：**抽测原 503 卡**；产品实现仍未 commit；非 shipped/online；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`routing_fix_uncommitted_spotcheck`；STATE `l2_routing_fix_uncommitted_2026_09_11`；updated_at 2026-09-11T01:27:00+08:00；AGENTS §七 已短注。

- 2026-09-11 P0 L2 热门24 pass8/fail_product15/timeout1（项目管家；ToIV 开发 tip；docs；无产品 SHA）：P0 L2 热门 **24** — pass **8** / fail_product **15** / fail_timeout **1**；pass：h3-i2v / t2v / fl2v / i2v-15s-fast / txt2img-basic / img2img-basic / wan-animate-2 / qwen-image-edit；主因：市场卡未钉专池 → 通用池 **503**；H3 upload 默认不上 **:8195**；真源 `.regen_tmp/app_test_matrix_p0/l2_summary.json` **勿 stage**；产品实现仍未 commit；非 shipped/online；下一步：开发修路由/上传对齐；产品 dirty 勿 stage；勿 stage `.regen_tmp`；无产品 SHA；本地 tip 无 push；status=`fail_product_majority`；STATE `p0_l2_hot24_2026_09_11`；updated_at 2026-09-11T01:17:00+08:00；AGENTS §七 已短注。
...
- 2026-09-10 animate2 unhide 已落地 508→510；admin soft-hide 可再上架（项目管家；ToIV 开发+能力缺口 tip；docs；产品 dirty 待 commit）：产品码仍 dirty（status=`pending_product_commit`）— apps.py admin `_visible`+PUT；seed 不覆已有 is_public；tests；**勿 stage**。生产已落地：core `--skip-web`×2；animate2×2 `wan-animate-2`+`rh-character-replacement-minimax-h3-7582cc8e` **已 unhide** → 公开 **508→510**；重启误开 avatar-talk/facedetailer/hunyuan-i2v/latentsync 已再 soft-hide。通用 P0 **80** 仍等 fill（`unhide_after_fill_p0.json`）；animate2 已从候选移出。v20 旁路：dedicated_aware_ok(H3)=**510**；ok_if_dedicated_animate2=**2**（均已 unhide）；with_animate2=**512**；真源 `dedicated_aware.json`+`dedicated_aware_animate2_v20.json` 勿 stage；v21+ 必跑 animate2 postprocess。产品公开约 **510**；无产品 SHA；status 产品=`pending_product_commit` / unhide=landed；STATE `admin_soft_hide_relist_wan_animate2_2026_09_10`；updated_at 2026-09-10T15:55:00+08:00；AGENTS §七 已短注。

- 2026-09-10 mass soft-hide unusable v20 ×388（项目管家；ToIV 开发 tip；docs；无产品 SHA）：用户拍板只留能用；清单 hide **398**/新藏~**388**；生产 soft-hide **388** → is_public=false；公开约 **896→508**（期望 510；API list_len 510，含 2 张非公开 keep）；keep=main_ok∪ok_if_dedicated_h3=**510**；验收 `post_soft_hide_verify_v20` **ok**：公开 **508** ⊆ keep **510**；漏网 **0**；keep−public=2；无补差；**不全量 v21**；真源 `.regen_tmp/.../soft_hide_all_non_usable_v20.json` + `soft_hide_all_unusable_v20.json`/`_applied.json` + `post_soft_hide_verify_v20.json` **勿 stage**；不列 388 ids；产品 dirty 勿提交；未写 h3/；无 MODEL_SOURCES 变更；status=`soft_hide_docs`；STATE `soft_hide_all_unusable_v20_2026_09_10`；updated_at 2026-09-10T14:22:00+08:00；AGENTS §七 已短注。

- 2026-09-10 soft-hide hard-block-only v20 ×10（项目管家；ToIV 开发 tip；docs；无产品 SHA）：仅硬阻、无缺节点 **10** 张 → is_public=false；公开约 **908→898**；ids `rh-acc-0951611393-511ac0` … `rh-acc-9431956482-7fa928`（短：511ac0 dc3518 bd7f4d 6244b3 2babaa 15deeb 0b9198 f7ba44 615ed6 7fa928）；规则=missing_nodes 空且缺模全 hard-blocked/no-source；真源 `.regen_tmp/capability_gap_scan_v20/actionable/soft_hide_hard_block_only_v20.json` **勿 stage**（已 applied）；不扫 v21；放宽版不需要；Gitee main `dae4ee8`；GitHub 仍不可达；未写 h3/；无 MODEL_SOURCES 变更；status=`soft_hide_docs`；STATE `soft_hide_hard_block_only_v20_2026_09_10`；updated_at 2026-09-10T14:05:00+08:00；AGENTS §七 已短注。

- 2026-09-10 capability_gap_scan_v20 完成（项目管家；ToIV 能力缺口 tip；docs；无产品 SHA）：OK 245（持平 vs v19，+0）；apps 908（−1 soft-hide ef8fd7，对齐 02737db）；blocked 663；dedicated-aware 510（0）；only_h3 265；worker 838/822/822；style_models=2；loras=337；ef8fd7 已从公开/blocked 消失；主 OK 持平；等长尾或产品拍板再扫；仍缺 Top 非 H3：银月/燕如嫣/梅凝/韩立南宫婉/peiling3(6) + model.pt(5) + Kook亚洲人像/Licon-MSR/bfs/new_flux-2-klein-9b(4)；Top 缺节点仍 H3/RH + TT/Comfly；真源勿 stage；status=`scan_complete_docs`；STATE `capability_gap_scan_v20_2026_09_10`；updated_at 2026-09-10T13:50:00+08:00；AGENTS §七 已短注。

- 2026-09-10 soft-hide `rh-acc-1004064770-ef8fd7`（项目管家；ToIV 开发 tip；docs；无产品 SHA）：Z-image真实感-电商高清文生图 → is_public=false；公开约 **909→908**；原因=仅缺 hard-blocked `beyondREALITY_beyondREALITYZIMAGE.safetensors`（UltimateSDUpscale 已清）；更新 24ae742「勿藏 ef8fd7」口径；同权此前 8851286018 已藏（910→909）；真源 `.regen_tmp/capability_gap_scan_v19/actionable/soft_hide_beyondreality_zimage_followup.json` **勿 stage**；未写 h3/；无 MODEL_SOURCES 变更；status=`soft_hide_docs`；STATE `soft_hide_ef8fd7_beyondreality_followup_2026_09_10`；updated_at 2026-09-10T13:46:00+08:00；AGENTS §七 已短注。

- 2026-09-10 capability_gap_scan_v19 完成（项目管家；ToIV 能力缺口 tip；docs；无产品 SHA）：OK 245（+1 vs v18）；apps 909（post soft-hide 8851286018）；blocked 664；dedicated-aware 510（+1）；only_h3 265；worker 838/822/822；style_models=2；loras=337；已清 Remix high/low + moodyRealMix + BEYOND；UltimateSDUpscale 清（node gap）；仍缺 Top 非 H3：银月/燕如嫣/梅凝/韩立南宫婉/peiling3(6) + model.pt(5) + Kook亚洲人像/Licon-MSR/bfs/new_flux-2-klein-9b(4)；Top 缺节点仍 H3/RH + TT/Comfly；真源勿 stage；status=`scan_complete_docs`；STATE `capability_gap_scan_v19_2026_09_10`；updated_at 2026-09-10T07:21:00+08:00；AGENTS §七 已短注。

- 2026-09-10 MODEL_SOURCES v17 长尾齐：ok465/blocked339/total804（项目管家；ToIV 开发+模型下载；docs；无产品 SHA）：前 tip 0616f0c 462/342/804（Δ +3/−3）；若 BEYOND 淡妆浓抹已计则相对 0f6a0a4 量级 +4/−4；newly_ok≈**49.55 GiB**：Wan2.2_Remix_NSFW high+low / moodyRealMix_zitV5DPO / BEYOND 淡妆浓抹 BF16；`beyondREALITY_beyondREALITYZIMAGE.safetensors` **blocked**（无精确公开源；**勿**冒充/等同淡妆浓抹 BF16）；soft-hide `rh-acc-8851286018-43497c` → is_public=false；市场约 **910→909**；**勿**藏 `rh-acc-1004064770-ef8fd7`（同缺该权重但还缺 UltimateSDUpscale）；未写 h3/；header 已同步 JSON；真源 `.regen_tmp/capability_gap_v17_download_summary.json` / `beyondreality_zimage_trace.json` / soft_hide actionable **勿 stage**；另：设备管家 — 主池三机已装 `ComfyUI_UltimateSDUpscale`（`ssitu/ComfyUI_UltimateSDUpscale` @ `a5547db9`；submodule Coyote-A @ `2322caa`；**无 pip**）；object_info 有 UltimateSDUpscale / CustomSample / NoUpscale / Guider；**未** soft-hide `rh-acc-1004064770-ef8fd7`（仅记节点已装，可能日后 unblock）。status=`inventory_and_soft_hide_docs`；STATE `model_sources_v17_beyondreality_soft_hide_2026_09_10`；updated_at 2026-09-10T07:18:00+08:00；AGENTS §七 已短注。
...
- 2026-09-10 soft-hide blocked-only 12（ToIV 开发；v16 后续；docs；无产品 SHA）：仅缺 hard-blocked 精确名且无其他节点缺的 **12** 张 → `is_public=false`；市场列表约 **922→910**；真源 `.regen_tmp/.../soft_hide_blocked_only_v16.json` 勿 stage；status=`soft_hide_docs`；STATE `soft_hide_blocked_only_v16_2026_09_10`；updated_at 2026-09-10T05:24:54+08:00；AGENTS §七 已短注。
...
- 2026-09-10 capability_gap_scan_v14 完成（项目管家；ToIV 开发 tip；docs；无产品 SHA）：OK 224→235（+11）；apps 924→922（软隐藏 −2）；blocked 687；dedicated-aware 491→500（+9）；only_h3 265；worker 817/798/798；已清 P0/P1 六项；仍缺命名 LTX-2.3-Licon-MSR(7)/new_flux-2-klein-9b(7)/bfs_head…rank128B(5)/gimmvfi(7，疑 folder)；Top 非 H3 ≤7；Top 缺节点仍 H3/RH + TT/Comfly；真源勿 stage；status=`scan_complete_docs`；STATE `capability_gap_scan_v14_2026_09_10`；updated_at 2026-09-10T04:32:00+08:00；AGENTS §七 已短注。
...
- 2026-09-10 capability_gap_scan_v13b 完成（ToIV 开发；docs；无产品 SHA）：OK 210→224（+14）；apps 924；blocked 700；dedicated-aware 477→491（+14）；only_h3 267；folder 目标全清（ltx spatial 1.1/1.0、MelBandRoformer_fp16、Qwen3.5-9B*）；MODEL_FOLDERS 含 latent_upscale_models/vocal_separator/MelBandRoFormer/LLM；新 Top 非 H3 缺模 ≤7 apps；Top 缺节点仍 H3/RH + TT/Comfly；真源勿 stage；status=`scan_complete_docs`；STATE `capability_gap_scan_v13b_2026_09_10`；updated_at 2026-09-10T04:15:00+08:00；AGENTS §七 已短注。

- 2026-09-10 LIVE：通用池 extrapaths 补 LLM/vocal_separator/MelBandRoFormer + llama-cpp 勿覆 LLM（设备管家；docs）：解开 v13 似 folder-mount still_missing（MelBand/Qwen3.5 相关；ltx 或仍另轨）；**未**扫 v13b。旁证：~~T8前 267→208/59~~ → T8后主 212/55、别名 265/2（见新 tip）；原 N3 2 不挡/T8 turbo 挡 1；结案仍 Y34/N3；真源旧/新 side_scan 勿 stage；status=`live_device_fyi`；STATE `genpool_extrapaths_llm_vocal_melband_2026_09_10`；updated_at 2026-09-10T04:08:00+08:00；AGENTS §七 已短注。
...
- 2026-09-10 capability_gap_scan_v12 完成（ToIV 开发/设备管家；docs；无产品 SHA）：OK 200→206（+6）；blocked 718；dedicated-aware 451→457；RMBG 缺模榜已清；真源勿 stage；status=`scan_complete_docs`；STATE `capability_gap_scan_v12_2026_09_10`；updated_at 2026-09-10T03:25:00+08:00；AGENTS §七 已短注。
...
- 2026-09-08 LIVE：通用池 P0 装 mixlab/Derfuu/QwenVL/PainterFlux/MemoryCleaner（设备管家；docs）：已装 mixlab/Derfuu/QwenVL/PainterFlux/MemoryCleaner；Text 同名未解（KayTool 未批）；status=`live_device_fyi`；STATE `general_pool_p0_custom_nodes_2026_09_08`；updated_at 2026-09-08T19:17:00+08:00；AGENTS §七 已短注。
...
- 2026-09-08 LIVE：MODEL_SOURCES batch13 计数更新（真机/docs）：`docs/MODEL_SOURCES.md` + `docs/MODEL_SOURCES.json`；ok **292** / blocked **159** / total **451**（前 262/130/392）；updated `2026-09-08T05:46:49+08:00`；status=`inventory_updated_batch13`；STATE `model_sources_inventory_2026_09_08`；updated_at 2026-09-08T05:47:00+08:00；AGENTS §四/§七 已短注。
...
- 2026-09-08 LIVE：家族模板 wipe1166 + 准确重入131 COMPLETE（真机/core；docs）：删 **1166**（rh-h3/rh-minimax*）；准确重入 **+131**；跳过 **360**（detail_code_901×289 + null_workflowId×71）；错误 **0**；直播 rh-acc ≈ **837**；抽检 **12/12**；KEEP 原 rh-acc + 产品 h3-* builtins；家族克隆 **0**；新 131 表单 repair updated91/unchanged40/with_select83；防复活已清空 `apps/api/app/data/rh_h3_presets.json`（bak 保留；路径仅记文档，勿提交）；status=`wipe_reseed_done`；STATE `rh_form_cover_fidelity_2026_09_08` + `rh_family_wipe_accurate_reseed_2026_09_08`；报告 `.regen_tmp/rh-family-wipe-accurate-reseed-20260908.md`（勿提交）；updated_at 2026-09-08T03:21:00+08:00；无产品 SHA；AGENTS §七 已短注；未推。

- 2026-09-08 LIVE：封面效果对齐完成（真机/core；docs）：rh-acc **全 RH CDN**；修 **35**（5 rh-acc mp4→still CDN + 30 builtin 上传 RH 图）；剩约 **18** 无 RH 映射 builtin 待真出图（禁 NSFW `/covers/generate` 渐变）；可见 apps≈**811**；rh-acc 期间 sibling reseed **706→763**；市场家族 `rh-h3` 克隆当前 **0**；家族 wipe/reseed **已完成**（见上条 wipe1166+重入131；rh-acc≈837）；status=`done_18_pending_real_art`；STATE `cover_effect_align_2026_09_08`；报告 `.regen_tmp/cover-effect-align-20260908.md`（勿提交）；updated_at 2026-09-08T03:21:00+08:00；无产品 SHA；AGENTS §七 已短注；未推。

- 2026-09-08 LIVE：RH 表单封面保真调研+拍板→wipe_reseed_done（真机/core；docs）：rh-acc **706** 修**168**/齐**529**/skip901=**9**/err**0**；封面 RH CDN **706/706**；LIST→select 修复；去 graph fallback；封面改写仅**2**；错位主因≈**1166** 家族模板克隆（rh-h3/rh-minimax* 无 webappId 本地海报）；**用户拍板已完成** wipe **1166**→准确重入 **131**；跳过 **360**；rh-acc≈**837**；抽检 **12/12**；家族克隆 **0**；keep=rh-acc+product builtins；status=`wipe_reseed_done` decision=`wipe_family_clones_accurate_reseed`；STATE `rh_form_cover_fidelity_2026_09_08` + `rh_family_wipe_accurate_reseed_2026_09_08`；报告 `.regen_tmp/rh-form-cover-fidelity-20260908.md` / `.regen_tmp/rh-family-wipe-accurate-reseed-20260908.md`（勿提交）；updated_at 2026-09-08T03:21:00+08:00；无产品 SHA；AGENTS §七 已短注；未推。
...
- 2026-09-08 LIVE：封面 coverless/gap_pending→0（真机/core；docs；UX 真理已滚动）：DB empty=**0**；local **1214**；CDN **949**；bad_files **0**；UX 渐变=抽象 cover（~68 R18）；H3 R18 SFW inherit×7；STATE `cover_gen_coverless_2026_09_08` + `rh_comfy_covers_dupes_audit_2026_09_08`；报告 `.regen_tmp/fix-comfy-load-covers-20260908.md` / `.regen_tmp/rh-comfy-covers-dupes-audit-20260908.md`（勿提交）；updated_at 2026-09-08T02:10:00+08:00；无产品 SHA；AGENTS §七 已短注。
...
- 2026-09-07 LIVE：RH 准确重入库第一批完成（真机/core；docs）：seeded=**200** skipped=**294** errors=**0**；跳过原因 null_workflowId/未开放；无模板克隆（exported Comfy API graph）；族 flux50/qwen42/ltx36/wan32/h3 6/other39；抽检 **8/8** 节点+封面+RH id；ids prefix `rh-acc-*`；无新 BUILD（HTTP 种库）；status=`reseed_batch1_done_more_pending`；STATE `rh_clone_purge_reimport_2026_09_07` + `rh_accurate_reseed_batch1_2026_09_07` reseed_pilot_batch1=done seeded=200 skipped=294 errors=0 spot_check=8/8 build_id=null no_template=true updated_at 2026-09-07T20:00:00+08:00；报告 `.regen_tmp/rh-accurate-reseed-20260907.md`（勿提交）；更多批次 pending 勿标全量 reseed done；无产品 SHA；未发明集群/GPU；AGENTS §七 已短注。

- 2026-09-07 LIVE：10Eros 模型出处补全（64/64）已上 core（真机/core；docs）：覆盖 62/64→**64/64**；flagged 10→0；wiki+engine_registry civitai/HF（H3 10Eros_Max… + LTX 10eros_v14；顺带 hunyuan_video / flux1-dev）；BUILD_ID 仍 `20260907-103808-6e94327-dirty`（本轮主要改 api）；准确重入库 PILOT 仍进行中勿标 reseed done；STATE `model_provenance_10eros_2026_09_07` + rh_clone provenance→64/64 updated_at 2026-09-07T19:50:00+08:00；报告 `.regen_tmp/10eros-provenance-20260907.md`（勿提交）；无产品 SHA；未发明集群/GPU；AGENTS §七 已短注。

- 2026-09-07 LIVE：RH 真工作流导出已打通；准确重入库 PILOT 进行中（真机/core；docs）：METHOD FOUND — Recipe POST /api/webapp/detail → workflowId；POST /api/openapi/getJsonApiFormat (apiKey+Bearer) → data.prompt=Comfy API JSON；verified 3+ samples（Flux2-klein 17 / H3 Lip Sync 22 / H3 T2AV 33 等）；limits workflowState=0 or author ACL → 1913/WORKFLOW_NOT_EXISTS；prior BLOCKER TOKEN_MISSION/wrong path SUPERSEDED for workflowId+workflowState=1；wipe 已报 rh-*=0 cumulative≈8611；reseed PILOT started（非全量）；reseed_count TBD 勿发明；完成后报告 reseed count / skip-reason stats / spot-check / BUILD_ID if deployed；未部署无 BUILD_ID；no_template_reseed=true；STATE `rh_clone_purge_reimport_2026_09_07` status=reseed_in_progress_export_ok reseed_started=true reseed_pilot=true reseed_count=null prior_blocker superseded；报告 `.regen_tmp/rh-graph-export-path-20260907.md`（勿提交）；无产品 SHA；未发明集群/GPU；AGENTS §七 已短注。

- 2026-09-07 LIVE：RH 清库完成（rh-*=0；重入库 BLOCKER；真机/core；docs）：rh-* 已清 0；本轮删 3392（API 1149 + SQL 2243）；累计≈8611；保留 48（47 builtins + Flux-文生图-96c82d）；出处 96.9%（62/64）；未证主因 10Eros_Max_h3_* / 10eros_v14（约 10 H3/LTX NSFW）；另曾标 hunyuan_video I2V + flux1-dev-fp8（pulid）；重入库 0；BLOCKER RH 无工作流图导出（detail 仅元数据；export 403 TOKEN_MISSION）；未部署无 BUILD_ID；SQL 因 admin is_builtin 403；no_template_reseed=true；STATE `rh_clone_purge_reimport_2026_09_07` status=wipe_done_reseed_blocked rh_remaining=0 deleted_this_run=3392 cumulative≈8611 retained_apps=48 model_provenance=62/64 reseed=0 blocker=RH_workflow_export_TOKEN_MISSION build_id=null deployed=false；报告 `.regen_tmp/rh-wipe-resume-20260907.md`（勿提交）；勿声称重入库完成；无产品 SHA；未发明集群/GPU；AGENTS §七 已短注。
...
- 2026-09-07 core RH 家族预设累计 730（历史；计数已 SUPERSEDED→1230；+batch3 500；含 VACE 120；无产品 SHA；docs only 未推）：pilot30 + batch2 200 + batch3 500（含 VACE 120）；X-NSFW 可见；非 builtin；rh_family_presets.json 已更到 730 仍未提交；代码侧未提交 rh_family_preset_seed.py + data/rh_family_presets.json + env TOIV_SEED_RH_FAMILY（默认关）；AGENTS 未动。
...
- 2026-09-07 Spark LLM/VLM 真机+core 双切验证（设备管家 SSH + ToIV 开发；docs only 未推）：spark02 `vllm_node` @ `192.168.71.84:8000` LIVE — Qwen3.8-27B-NVFP4 served `qwen3.8-27b`（别名 `qwen3.6-uncensored`），max_model_len 32768；设备冒烟 ~15.4 tok/s；core 路径冒烟 ~27 tok/s；MemAvailable ~2.7Gi。双机 qwen38sg Flash-Next **Exited**；spark01 :8000 API down（~117Gi free）；LiveKit 仍在 spark02。**core 已切**：`TOIV_LLM_*` / VLM → `.84:8000` + `qwen3.8-27b`；toiv-api 重启 LAN health 200；备份 `.env.bak-qwen27b-20260907`。Embedding→spark01 **DONE**（见上条 LIVE）。此前 intent_only /「保留 Flash-Next」/「路由改中」**SUPERSEDED**。AGENTS 设备表+§五 LLM/VLM 已改 LIVE。
...
- 2026-09-07 P0 主题系统 v9 ON CORE dual-pushed tip `4d9e583` (product `19d65db`): presets minimal/cinema/paper/graphite + custom accent; BUILD_ID `20260906-202535-4d9e583-dirty`; api :8090 / web :3100 200; verify only core LAN 192.168.71.47:3100 (no public tunnel); shots /tmp/theme-prod/ + .regen_tmp/theme-prod/; AGENTS.md untouched; market merge not started.

- 2026-09-06 应用市场 RunningHub 化（未 commit 未部署，工作区改动）：①市场页深黑作用域 `.rh-dark`（apps.css RH-ACCENT 标记块定义 `--rh-lime:#c9f24f` + 深黑 #0b0d10 系，作用域内覆盖全站 token 让工具栏/chips/ParamField/Empty 自动落暗，accent 换荧光绿——at-btn--primary 在作用域内即荧CTA；hex 只许在标记块，两处 token 纪律测试已同步豁免口径）；②瀑布流封面大卡：`.rh-grid` CSS columns 5/4/3/2（断点 1599/1199/767），renderCard 全重写为 AppCard——封面充满整卡（`cover_url` 经 imageUrl 带 token，onError/空值降级按 category 色相暗渐变+居中大图标，占位高度按 id 散列 4 档成瀑布流），底部 scrim 渐变压黑 + 白字标题 600 + 作者行（首字母圆头像 + `author` 兜底「ToIV」）+ mono ▶ `usage_count`（唯一真实数据，无点赞/收藏造假）；fork/R18/我的收进角落小标，hover 封面 scale(1.04)+荧光描边+「运行」pill；③详情页（AppRunnerView）重构两栏：>1023px 左 380px 参数列（sticky+内滚；`groupAppParams` 三档折叠分区：素材上传/提示词/生成参数，荧光标题+chevron；number 走新 `RhNumberField` −/+ stepper，其余复用 ParamField）+ 通栏荧光「立即运行」大按钮（rh-run-btn，禁用原因/运行状态在按钮区），右列 rh-preview（cover 大图/占位 + 简介）+「我的生成」（listJobs 按 `app_id` 过滤 done 产物网格，本次结果置顶，ResultTile 公共组件）；顶条保留「简洁/工作流」段控（叠 rh-seg 荧光 pill），工作流=AppWorkflowGraph 画布不变；④壳层 MarketView 段控叠 rh-seg（自引 apps.css）；⑤数据契约增量：AppItem +`cover_url`/`author`（可空宽容归一），JobItem +`app_id`，后端 `_job_dict` 从 params 快照解析透出 `_app_id_of`（与 batch_id 同范式，纯增量键）。回归：npm test 928/928（新增 tests/appsRh.test.ts 13 例；appsViews/appsImportModal 断言适配）/ tsc 0 / `rm -rf .next` 干净构建通过；后端 pytest test_jobs+test_apps+test_apps_run+test_duration 141 绿。截图（本地 dev 3100 + 隧道 18090→core 8090 真实数据，admin 登录）：/tmp/rh-market-light-shell.png（亮壳不冲突）/rh-market-dark-shell.png/rh-market-hover.png（运行 pill+荧光描边）/rh-detail.png/rh-detail-full.png（两栏+我的生成空态）/rh-detail-narrow.png（900px 堆叠）/rh-detail-workflow.png（画布模式）/rh-create-video.png（视频创作页内嵌目录回归）。已知偏差：生产 core 后端尚未下发 cover_url/author/app_id（并行开发中），截图验证的是占位封面路径；「1 issue」红标为 dev overlay 的 data-mode 水合告警（截图脚本强写 toiv_mode 所致，layout 内联脚本预存行为，非本次回归）。

- 2026-09-06 单色极简改造（`38145e7`，已部署 core，生产截图核对）：用户指令「最近作品删掉/主体只黑白/多余文字全删/只留最简功能」。①单色化：--accent-glow 亮暗双双重定义为中性（text-primary 近黑/近白），soft/deep 派生同步——聚焦环、at-seg 指示、链接 hover、启动序列、SideRail 指示条、进度条、形象卡选中环、阶段条完成段、charts 单色全部自动黑白化；语义状态色与 --chart-* 保留。②首页（AssistantView 门户）：删最近作品整块（含 listJobs 拉取）、快捷 chips 行、模型行、TOIV 铭牌、副文案；场景卡改无边框文字行（描述收进 title tooltip）；合规行保留（最小弱字）。③文案清扫：ResultPanel 空态只留「产物将在这里呈现」；ParamField 全字段类型的长 hint 说明段删除（影响 AppRunnerView/AppImportModal 同组件）；融合行尾能力 tags 删除；select 选中项底模一句话简介保留（功能性）。测试同步 4 文件断言反转。回归：npm test 915/915 / tsc 0 / 干净构建；生产截图 home/fusion 核对通过。UI_STANDARD §2.2 等四处口径已同步单色化。

- 2026-09-06 workstation 重启窗口 + spark 集群重建（全部真机验证）：①workstation 冷启动后 GPU0 恢复 CUDA 枚举（index 0）、8189 卡死进程清除；落卡复核 gpu0-alt/joycaption/longcat→GPU0（CVD=0 归位）、H3/infinitetalk/fishs2→GPU2（UUID 钉）、四音频→GPU2；NAS 挂载正常；LB /admin/backends 三后端 healthy；core /api/health ok；txt2img 冒烟 success（pc01）。②spark01 死机（WoL 无效，物理重启）→ SGLang 集群重建：容器 qwen38sg 的 /tmp bind-mount 随 reboot 丢失（tmpfs 教训再实证，docker 启动失败时还误建 root 空目录），从镜像临时容器救回文件后**重建容器、挂载源改家目录**；decode 连崩三层——SM120 误开 TMA（上游 flash-attn #2671）→ pack_gqa 半成品（#2656）→ **head_dim=256 在 GB10 99KB smem 上 FA4 cute 不可用**（#2750 同类；repro 实证 hdim=256 全挂、128 全过）→ 最终方案：FA4 整包升级 4.0.0b29（PyPI 纯 py 轮，目录级挂载 /home/dgmt-spark/fa4pkg/flash_attn）+ qwen_sparse_attn_backend 打补丁（SM12x 且 hdim>128 的 decode 改走 PyTorch SDPA 块对角 bool mask 路径——首版逐请求循环因 `int(cu_k[b])` host 同步在 CUDA graph capture 下 invalidated，改 GPU 构图后通过）。③验证：/v1/models UP，chat 出文正确（usage 正常），core→spark /api/optimize 提示词优化链路出英文优化词正常。④注意：spark01/02 无免密 sudo；/tmp root 空目录残留无害；重启窗口期间 AIGCPannel 网关若报 LLM down 属预期。

- 2026-09-06 设备管家 beijing SSH 公钥已通：MateBook 已装 id_ed25519 公钥；~/.ssh/config 有 Host beijing → 8.140.222.24 User root IdentityFile ~/.ssh/id_ed25519；BatchMode 免密已通。密码不入文档。

- 2026-09-06 设备管家 beijing 真机只读登入回写：8.140.222.24 hostname `iZ2ze325an97cwlbt1wxfdZ`；Ubuntu 24.04.4 LTS；1.6Gi RAM 无 swap；40G 盘约用 15%；角色确认 CN 入口 toiv.wineryz.top（OpenResty+ACME）；Docker `1Panel-openresty` + `1Panel-frps`（frps 0.68.1）听 7000/7500/13100/18090，另有 `1panel-core` :1722；SSH（**已 supersede**：当时公钥不行/无 Host，现 id_ed25519 + Host beijing + BatchMode 已通）root 密码可登、MateBook 默认 id_ed25519 公钥不行、~/.ssh/config 尚无 beijing Host；未改服务；密码不入文档；Tailscale 仍 —。

- 2026-09-06 遗留清单全量收口：①6 例预存测试失败修复（`55c3902`）——根因：133f15a「H3 市场应用走专用实例 :8195」后 test_app_seed/test_agent_gen_tools 旧打桩只拦 pool.pick，请求被路由到真实生产 H3（t2v 用例曾真实提交作业）；补 ensure_h3_enabled/pick_h3_client/ensure_h3_ready/ensure_h3_vram 四处接缝 stub，全量 **3035 passed 0 failed**；②8189 卡死收口——sudo ss -tlnpe 查 cgroup 实锤占用者是 comfyui-gpu0 unit 内僵尸进程 3233 的失控线程 13717（R 态，kill -9 无效，与 GPU0 requires reset 同源），`systemctl disable comfyui-gpu0`（防重启复活/双开），**:8196 gpu0-alt 转正**,8189 退役挂重启窗口；③GPU0 reset 实测 `nvidia-smi --gpu-reset -i 0` → Not Supported（RTX PRO 6000 工作站卡不支持），只能重启；⚠️ 重启后 CUDA 序号重新包含 GPU0，数字 CVD 服务（gpu0-alt/joycaption/longcat/四音频）落卡会偏移，UUID 钉卡服务不受影响，重启后须逐个复核；④Cloud SSH：公网 :22 握手挂 2min 被 LoginGraceTime 切断（跨境疑似 QoS），Tailscale 路径可通但 21s——`GSSAPIAuthentication no` 修复后 5.4s，SSH 一律走 100.83.78.114；⑤DRT 包六件（source-v2/images/config/pg/redis/minio）已中转 staging 至 core `/home/merlin/drt-bundle/`，执行归 DRT 负责人；⑥drama 播放器生产截图验证（剧集 2a294cad，页面结构/控制条/信息面板正常）；⑦AIGCPannel 侧 .env 旧值注释停用 + 文档残留 :8189/.115 全修；⑧UI_STANDARD 追认三处琥珀触点（进度条细线/落地页关键词/选中与完成语义）。

- 2026-09-04 全站美化方向A「精修控制台」五波全部收官（commits `33c045c` Wave0 / `2393957` Wave1 / `79bd123` Wave2 / `2988b9c` Wave3+4，均已部署 core，BUILD_ID `20260904-101401-2988b9c-dirty` 真机核对一致）：Wave0 版型令牌系统（--layout-measure/content/wide、节奏/栅格/媒体宽高比档）+字阶/圆角旧档删除+琥珀点睛令牌+accent-halo 更名+装饰层裁决（噪点减半/全局暗角收进舞台/启动序列单色化）+UI_STANDARD v2.0；Wave1 浮层统一 --shadow-pop+弹簧入场、空态三档（stage/section/inline）、charts 单色琥珀、控件 36px 规范；Wave2 门户首页重设计（Fraunces 问候+琥珀 italic「创作」+场景卡+最近作品带）、作品库画廊节奏（图像 1/1 / 视频 16/9 混合 AR）、融合行琥珀指示、生成三视图舞台空态+PromptBar 聚焦浮起；Wave3 七创作工具视图（image-edit/video-edit/animatic/avatartalk/canvas/dub/studio）版型对齐+空态升级+阶段条三态；Wave4 市场/资源/设置/观测/admin×3/落地/agent-runs/drama 全量收编（含 skills.css 204 行重复 styled-jsx 删除、死令牌 --radius-card/--text-caption 修复、P-2b 一例）。回归：每波 npm test 915/915 + tsc 0 + 干净构建；生产截图验证（home/library/video/fusion 等，亮暗双模式）。Film Atelier 复兴 demo（方向C）被用户否了（效果一般），走 A 方向；demo 留档 .regen_tmp/film-atelier-demo.{html,png}。已知偏差：观测台保持 wide 档未收窄；drama 播放器无真实剧集截图（仅 CSS 令牌替换+构建验证）；训练进度条/落地页关键词两处琥珀触点待 UI_STANDARD 追认。

- 2026-09-04 全站美化方向A·第 2 波 A 组（已并入 `79bd123`，已部署 core）：image/video 生成视图 + 音频视图 + 应用目录卡（共享 stage.css）。①空舞台接 at-empty--stage：stage.css 新增 `.stage-empty` 适配层（绝对定位铺满舞台 + 琥珀 5% 聚光径向渐变 + at-empty--stage 去重框/底），ResultPanel 与 AudioView 三工具（TTS/ASR/分离）空态全部换 `Empty size="stage"`（琥珀线稿图标 + Fraunces 斜体一句 + 引导语），v1 单行提示 `.empty-console-hint` 类删除（entities/settings/library/observability 仅注释引用，已改为指向 at-empty--inline）；②PromptBar：focus-within 微浮起改 -1px + `--shadow-pop` + 琥珀细环（border-color `--accent-glow`，呼吸光晕改 glow-soft 3px），chips 行间距 `gap: var(--space-1) var(--grid-gutter)`；③结果区：下载钮自左上状态胶囊迁入舞台右下 hover 操作浮层 `.stage-media-actions`（玻璃胶囊，opacity 淡入 ≤200ms，触屏 hover:none 常显），生成骨架默认宽高比 16/10→`--media-ar-video`（有目标尺寸快照时仍内联覆盖真实比），胶片条状态角标偏移 5px→`--space-1`；④参数列节奏：浮板头部下距 `--layout-header-gap`、body 段距与 hairline padding 统一 `--section-gap`（recipes-section 同步）；⑤应用目录卡（apps.css + AppMarketView/AppRunnerView）：`.apps-card` 挂共享类 `.at-card--interactive`（整卡点击=打开，按钮/链接/划选文本守卫），键盘聚焦态琥珀描边 1.5px（1px border + 0.5px 环）+ soft 底；卡片网格 gutter→`--grid-gutter-lg`；结果卡媒体统一宽高比（`.apps-result-frame--video` 16/9 / `--image` 1/1，contain 保真 + surface-1 信箱底），下载操作改 hover 浮层淡入（音频卡保持流内）；KindCreateView 壳段控下距 `--layout-toolbar-gap`；⑥硬编码收编：GenerateView styled-jsx（5px/3px 6px/999px/11px/16px/50%→token，死样式 entity-ref-hint/motion-brush-hint 删除）、AudioView styled-jsx（34px 工具行→36 工具条行高档、2px→--space-1、指示条偏移 token 化、640px→--layout-measure、移动档 44px→--touch-target）。测试断言同步更新 3 文件（uiPolishViews 空态范式、generateInspector 空态+新类名、audioEditStage 三工具 StageEmpty）。回归：npm test 915/915 / tsc 0 / `rm -rf .next` 干净构建通过；Playwright 截图 18 张（/tmp/wave2a-*.png：视频空态/有结果/hover 浮层、图像有结果、音频编辑空态+生成 tab、avatar-talk 回归、应用目录 hover、PromptBar 聚焦 × 亮暗；本地 3100 生产构建 + SSH 隧道 8090→core 真实 API，有结果态素材为 localStorage 注入真实签名产物路径）。偏差说明：结果区 hover 浮层只做「下载」（会话条目无详情页可跳）；avatar-talk 对 stage.css 的消费仅 `.adv-params`（本波未动），截图回归无破版。注意：工作期间 assistant/fusion/library 6 文件出现并行代理（疑 Wave2 他组）改动，本组未触碰、未回滚。

- 2026-09-04 全站美化方向A·第 0 波（`33c045c`，已部署 core）：设计令牌收敛 + 版型系统 + 装饰层裁决 + 规范重写。①版型令牌新区（globals.css）：`--layout-measure/content/wide` 680/1000/1240（--content-max/--content-wide 改别名指向新档，引用不断裂）、节奏档 `--layout-header-gap`/`--layout-toolbar-gap`/`--section-gap-lg:24`、栅格 `--grid-gutter:16`/`--grid-gutter-lg:20`、媒体卡 `--media-ar-video/image/thumb`（16/9、1/1、4/3）；②字阶收敛：旧数字档 `--text-xs~2xl` 全站 88 处引用机械映射（xs→aux/sm→body/base→body/md→section/lg→title/xl→display-sm/2xl→display-md）后删档；移动端例外：`.input` 钉 16px（iOS 缩放硬约束）、`--text-aux` 移动档 12px 保留；③圆角收敛：`--radius-xs~xl` 90 处映射（xs/sm→badge、md→control、lg/xl→panel）后删档，`--radius-full` 保留；④`.page-header` 双定义合并为单块（flex 底对齐+分隔线同源，下距走 `--layout-header-gap`）；⑤`--shadow-pop` 浮层专用双层柔影成文（本已存在，补注释定位）；⑥品格色点睛：原近黑 `--accent-glow` 更名 `--accent-halo`（52 处机械改名，视觉零变化），`--accent-glow` 让位琥珀 `#b07d2a`/暗 `#d4a94e` + `-soft`/`-deep` 派生，触点仅三处——`--focus-ring` 琥珀、无类名文本链接 hover（`a:hover:not([class])`，带类控件不受影响）、`.at-seg` 激活段内底缘 2px 指示线；⑦装饰层裁决：噪点暗色 0.03→0.02（亮色本已 0.02），全局 `body::after` 暗角删除，暗角收敛进舞台容器（stage.css `.stage-main::after` 原有 + avatartalk.css `.at-stage::after` 新增同配方）；⑧assistant 启动序列单色化：cyan #22d3ee / violet #a78bfa 全量替换为 `--accent-glow`/`--accent-glow-deep` 琥珀单色系（neon-edge/neon-core-arc/neon-core-dot/av-panel-aura/av-boot-core），动画时序逻辑零改动；⑨docs/UI_STANDARD.md 重写为 v2.0（v8 现实对齐 + §4 版型系统成文 + at 系唯一范式 + 装饰层规则 + §13 行数表改视图职责表）。测试断言同步更新 3 文件（themeMode 暗角裁决、themeContrast 版型令牌、assistantPopupNeon 单色化）。回归：npm test 915/915 / tsc 0 / `rm -rf .next` 干净构建通过；Playwright 截图 6 张（home/video/library × 亮暗，/tmp/wave0-*.png，本地 3100 + SSH 隧道 8090→core api）目检无意外回归，at-seg 琥珀指示线双模式生效。已知视觉变化（映射副作用）：旧 sm/base 文案 12/14→13、旧 md 14→15、旧 lg 16→20（ImageEditView/animatic/DubView 各 1-2 处小节标题）、旧 xl/2xl 数字 20/24→24/32（AdminView 统计数 1 处）、4px 圆角→6px、16px 圆角→12px（stage.css 两处）。风险：琥珀 a:hover 文本对比度 ~3.4:1 低于 AA 4.5（hover 增强态，定性可接受）；`--accent-glow` 语义重定义，若有缓存旧 CSS 与新 HTML 混部会短暂出现琥珀淡边（发版注意同批替换）。

- 2026-09-03 端点统一 + ComfyUI 池动态切换上线（`3446b75`+`7edabc5`，已部署 core + workstation LB）：①LB（代码在 AIGCPannel 仓 platform/deploy/comfyui-lb/）后端列表外置 `/opt/comfyui-lb/backends.json`，health loop 每 5s 查 mtime 热重载（零重启、不丢 prompt_map），新增只读 `GET /admin/backends`；②ToIV api `TOIV_COMFY_WORKERS_REGISTRY_URL`（core env 已配 `:8188/admin/backends`）→ WorkerPool pick 路径 60s TTL 惰性跟随，拉取失败/空沿用现有成员（LB 挂不炸池），注册表回环地址自动改写到注册表主机（**生产实证抓获**：LB 报 127.0.0.1:8196，core 不可达白白熔断，`7edabc5` 修）；③InfiniteTalk :8201 补 resolve_worker 精确匹配（E-1 核销）；④fleet_registry 展示层 :8189→:8196。AIGCPannel 侧三处漂移清单同步收口（未提交，见其仓工作区）。真机 e2e：JSON 加假后端 `fake-test` 5s 内 /admin/backends 出现且 UNHEALTHY、删回即消失、全程零重启；txt2img 冒烟两发（pc01、gpu0-alt :8196）提交正常、后者在 :8196 queue_running 执行（GPU0 显存紧张模型加载慢属正常）。测试：新增 12 例（pool registry 11 + deps 1）；后端全量 3007 passed / **6 failed 预存**（test_agent_gen_tools H3 mocked submit + test_app_seed 5 例，git stash 干净树复现与本次无关，疑 09-03 应用市场批次引入，待查）；LB 侧 8 例（AIGCPannel 仓）。操作新口径：改池后端只编辑 backends.json，禁止改 core env / LB 源码切成员。

- 2026-09-03 状态同步核验（本会话 SSH 真机）：HEAD `113e73b`，Gitee+GitHub 均 0 未推，工作区干净（仅未跟踪 .regen_tmp/、dogfood-output/）。core toiv-api/toiv-web 双 active、`/api/health` 200、前端 BUILD_ID `20260903-020621-262bb5a-dirty` 与记录一致。**发现口径漂移**：ComfyUI-LB 本地池后端文档写 :8189，真机已切 **:8196**（新 `comfyui-gpu0-alt.service`，同 GPU0/cache-lru 8，active+enabled）；旧 `comfyui-gpu0` :8189 反复启动失败 `Port 8189 is already in use`（LISTEN 残留、归属进程对 merlin 不可见、无免密 sudo 未查实），unit deactivating。core `TOIV_COMFY_WORKERS` 实况 = 127:8196 + 116:8188 + 114:8193。事发时另一会话正在 workstation 操作（journal 18:57），按 P-5 只记录未动手；占用者清理/回切 :8189 待处置方收口；GPU0 reset 仍 open。GPU 快照（MiB used/97887）：0=27275 / 1=94125 / 2=38159 / 3=84997。AGENTS.md 已修正 GPU0 行/LB 段 + 新增 H-7 + 近期关键变更条目（应用市场 262bb5a/133f15a、W1–W4 92a8bba…4922bca）。
...
- 2026-09-03 H3 apps/presets wrap-up (UNCOMMITTED, UNDEPLOYED, no product SHA) (superseded locally: 262bb5a committed, still not on core): video featured includes 15s-fast / r2v-voice and nsfw twins. H3 worker object_info has MiniMaxH3ReferenceToVideo. API focused tests 219 passed; web npm test 910 passed. Mini program still engine-first (no apps API client, no new architecture). 1262 community JSON not imported; no scene cards; no fake 20s. STATE not thickened — wait for commit SHA or deploy. AGENTS.md untouched.

- 2026-09-03 H3 apps/presets three pieces landed locally (UNCOMMITTED, UNDEPLOYED, no SHA) (superseded locally: 262bb5a committed, still not on core): (1) image/video KindCreateView app-first, advanced engines stay on GenerateView; H3 cards sorted first. (2) presets h3-t2v-15s-fast / h3-i2v-15s-fast / h3-r2v-voice and nsfw twins; no fake 20s. (3) assistant tools list_apps, get_app, run_app; submit_generation app-first; h3-fl2v/h3-r2v in _DISPATCH; skill h3-app-catalog. Tests: presets 24, app views 54, assistant-related 63. STATE not thickened — wait for commit SHA or deploy. AGENTS.md untouched.

- 2026-09-03 INTENT ONLY (code not landed) (superseded locally: three pieces landed uncommitted, still wait SHA to thicken STATE): RunningHub H3 workflows → ToIV apps/presets, not another engine wrapper. Generate page selects apps; assistant list_apps/run_app. 1262 community cards not 1:1 graphs — capability families (txt2v/img2v/first-last/omni-ref/turbo/voice-ref) as apps+presets. In progress: video-entry apps first (advanced engines kept), assistant list_apps/run_app, H3 preset seeding. AGENTS.md untouched. Wait for SHA before STATE details.
...
- 2026-09-03 app-run list binding correction (UNCOMMITTED, UNDEPLOYED): h3-r2v/h3-nsfw-r2v images→LoadImage 110–118, video→120/122/124, audio→130–132; extra slots stripped from graph. h3-fl2v still images+last_frame→100/101. pytest test_app_seed/test_apps_run/test_apps 75 passed. Marketplace UI then had no multi-image upload (superseded: upload wired); generate page images.max=9 unchanged. Files: apps.py, app_seed.py, apps/web/lib/apps.ts + tests. AGENTS.md untouched.
...
- 2026-09-03 Studio Console W4 死代码大扫除(commit 4922bca,已部署+生产实测 6/6):①drama 全链退役——components/drama/(11 文件)+drama-workbench.css(1919 行)+hooks/useDramaProject+tests/dramaWorkbench.test.ts,合计 -9329 行;六测试文件断言清理(uiLayoutBatch2/themeContrast/themeMode/uxBatchD 删例、autoResizeInputs 接线清单 17→14、progressWiring 删 autorun 段、uiCViews 两例改 LoadingBlock/ErrorBar 基座断言保留价值);app/drama/[id] 播放器路由独立(DramaPlayer.module.css)不受影响。②cornernav.css→nav-account.css 更名:CornerNav 早退役名不副实;剥除 fixed 右上角定位/玻璃触发钮/窄屏媒体规则等死样式(SideRail 底槽覆盖块已承载触发钮外观),375→约 230 行;layout.tsx 与 taskCenter.test.ts 引用同步;补回触发钮 flex 居中公共基座(原规则夹带,剥除时复核抓获)。③PageHeader 组件保留:studio/avatartalk/animatic/dub/admin×2 六处活消费方,转版属未来扩展非死码;library.css 头注释随 W3 更新。回归:npm test 891(drama 专用测试随删,净 -55)/ tsc 0 / 干净 build。生产实测 6/6(Playwright 1440×900):侧栏底槽双钮 36px 同形态无玻璃、账户/任务弹层上翻正常、资源中心三 tab 无大标题、控制台零真实错误、drama 入口零残留。至此 Studio Console W1-W4 全部收官。

- 2026-09-03 marketplace builtin seed 7→36 (UNCOMMITTED, UNDEPLOYED): apps/api/app/services/app_seed.py + tests/test_app_seed.py; pytest 65 passed. Original 7 kept and holes patched: h3-t2v, h3-i2v (LoadImage node 100 / inputs.image plus length/steps), txt2img-basic, img2img-basic, ltx-txt2video, ltx-img2video (image), ltx-lipsync (image+audio). New 29: h3-multishot, h3-nsfw-t2v, h3-nsfw-i2v, nsfw-txt2img, nsfw-img2img, qwen-image-edit, flux1-nunchaku, ltx25-multishot, wan-nsfw-i2v, wan-animate, wan-animate-2, wan-vace, longcat-t2v, longcat-i2v, avatar-talk, ovi-t2v, ovi-i2v, phantom-s2v, ace-music, ace-music-legacy, inpaint, upscale, removebg, controlnet, ipadapter, pulid, facedetailer, hunyuan-i2v, latentsync. Aliases: txt2img/img2img → *-basic; ltx-nsfw-* → existing ltx-* trio. Not listed (no honest single-form graph): H3 Ref2VA 9-ref, H3 first-last frame, H3 voice clone, longcat-continue, wan-transition, keyframe-chain, vace-edit. hunyuan3d omitted (output_kind has no glb). Production toiv.db needs API restart for seed_builtin_apps to write new cards. AGENTS.md untouched.

- 2026-09-03 Studio Console W3 六视图套版(commits 92a8bba+892fa95+3a27ca7,已部署+生产实测):字阶 token 全局落地正文 13/辅助 11;页头全灭——settings(刷新钮并入引擎卡标题行)/market 聚合(段控窄行)+Runner(36px sticky 细顶条)/entities(Tabs+新建钮工具条)/library 双页头(计数入工具条尾,回收站细顶条)/observability(实时指示收工具行)/resources 聚合+models/train/backlot 三内嵌(各收窄工具行);空态全单行化(Empty 组件在这些视图退役);fusion bento 版型(渐变洗光/水印/旗舰徽标/CTA/错峰入场)废弃改 hairline 分行紧凑列表;去玻璃(lib-toolbar/lib-batchbar 实底化)+卡片阴影/lift/媒体放大/错落入场/shimmer 扫光全退役;观测单色化(KPI 渐变文字/VRAM 渐变条/青紫 chart 色收敛为作用域覆盖,伪 token --text-11/12/14、--ease-out 修正)。复核抓获:settings 内容偏好区 at-card 残留+引擎标签色值遭 IDE 缓冲区回滚(生产实测抓获补打);非管理员撞观测台门控走 changeView→router.replace 引发 RSC 重试风暴 97+ 次,改 history.replaceState 客户端弹回。回归:npm test 946 / tsc 0 / 干净 build ×3;后端零改动。生产实测两轮全过(观测台需 email=admin 账户,role=admin 的其他邮箱也被门控弹回)。W4 遗留:drama-workbench.css 1919 行死链、cornernav.css 语义错位、少量死 .page-header 规则。

- 2026-09-02 音频编辑 tab 舞台化(W2 收尾,commit ec02660,已部署+生产实测 8/8):修复上轮勘察遗留「音频页编辑 tab 卡片表单范式 vs 舞台范式分裂」。旧编辑 tab 为三卡堆叠(ToolCard 基座,880px 居中单列滚动),与图像/视频「左参数列+中央画布」专业范式不一致。重构:①三工具状态上提为 hooks(useTtsTool/useAsrTool/useSeparateTool)驻留 AudioView 层——切换工具不卸载、不丢草稿(旧卡片条件渲染会丢);②左参数列 340px 面板岛:列顶工具切换列表(icon+名称三行,激活项 accent-soft 底 + 2px 左指示条沿用 SideRail 当前项语言,简述收进 title),中部表单滚动区(简述行+字段),主操作钉列底 hairline 分层;③中央舞台面板岛(与 generate-results 同款 surface-1+hairline+radius-panel):空态复用全局 empty-console-hint 单行提示(与 ResultPanel 空态同语言,按工具分文案)、进行中 TTS/分离走 StageBusy(全局 loading-spinner+状态行居中)、ASR 走 ToolProgress 限宽 420px 居中、结果区限宽 640px 居中滚动;④ErrorBar 置舞台顶(原夹在卡内表单与结果之间);⑤响应式:<1024px 纵向堆叠、工具切换横排、舞台 min-height 320px;移动端触控 ≥44px。行为保留:ASR 选新文件清空旧转写(回迁时一度遗漏,tsc 无感、自查补回)。退役:ToolCard/audio-tool-card/audio-tab-edit 及卡片 hover 升浮等样式。回归:pytest 2983(2 例 lipsync/drama 偶发,单独重跑即过,与前端无关)/ npm test 945(+4 新 audioEditStage:骨架类名/hooks 驻留/旧范式清除/三态齐)/ tsc 0 / 干净 build。生产实测 8/8:340px 参数列/三工具行+2px 指示条/钉底主操作/三空态文案切换/表单随工具切换/草稿驻留(TTS 输入切 ASR 再切回保留)/控制台零真实 JS 错误。签发实测 token 抓获坑:create_token sub 必须是 user.id 且需 source 生产 deploy/.env(裸跑用默认密钥签发必 401)。gitee+github 双推(github 补上前次超时账),deploy.sh → core 两服务健康。

- 2026-09-02 工作流画布可用性优化(commit 90fcb94,已部署+生产实测 7/7):用户问「工作流怎么正常使用」并要求针对场景优化。五处:①浮动运行条(画布右下玻璃条,改完参数就地跑,不再滚出画布;简洁模式保持底部提交条)②「可调 n」聚焦导航(循环定位绑定节点,自动平移缩放居中 + 1.6s 光环脉冲,prefers-reduced-motion 降级)③双击节点头聚焦放大至 100% ④hover 节点高亮上下游连线(is-hot 加粗 accent)⑤Save/Preview 系输出节点自动标「出口」绿徽标。自抓 bug:注释里写 `Save*/Preview*/` 的 `*/` 提前终止块注释导致语法错误(tsc 抓获),注释文本避开。生产实测:可调 4 循环聚焦/出口徽标/浮动运行条/hover 连线/双击聚焦/工具栏全过,控制台零 JS 错误。回归:npm test 941(+2)/ tsc 0 / 干净 build。github 推送当夜持续超时(镜像,gitee 与生产已最新)。

- 2026-09-02 工作流模式 v2:ComfyUI 节点图画布(commit 2cd7dbb,已部署+生产实测):用户要求类 RunningHub 节点图。关键约束:存的 workflow_json 是 API 格式(导入时 UI 格式已转 API,坐标丢失)——前端自动布局:最长路径分层成列 + 列内父/子重心两轮迭代减交叉(Sugiyama 简化),内置/已导入/未来应用全部适用,零后端迁移。渲染:SVG 贝塞尔连线层在下 + HTML 节点层在上(同一 transform,表单可交互);交互:拖空白平移、滚轮指针缩放(0.2-2x)、+/-/适配全图;节点卡 #id+class_type mono 头、绑定参数内联编辑(accent 描边+可调徽标)、只读行 mono 小字、连线 ← #id、绑定行连线汇入头部端口。生产实测(Flux2 文生图 11 节点):左→右流向正确、无重叠、连线清晰、平移/缩放/复位/跨模式值保留全过,控制台零 JS 错误。回归:npm test 939(+2:layoutWorkflow 分层/端口/环兜底,渲染断言改写为数 wf-edge——图标 SVG 的 path 会干扰)/ tsc 0 / 干净 build。

- 2026-09-02 平台能力全景入 RAG(commit 7e4d491,已部署+生产实测):用户要求把项目所有功能/页面/调用方法做成 RAG 交给 AI 助手。勘察发现向量 RAG 基建早已在位(rag.py:knowledge/*.md 按 `## ` 切块 → Qwen3-Embedding-4B @workstation:9302 → 余弦 top-4 每轮注入 system + search_knowledge 工具),缺的是能力语料。新建 knowledge/platform-capabilities.md(84 块 +8):18 页导航总览、任务食谱(「把视频翻译成英文」→dub、「做数字人」→avatartalk、「训练模型」→resources 等 20+ 意图映射)、23 引擎速查(图4/视16/音3,含何时用哪个)、内置应用清单、助手行动规矩(navigate_view/prefill_generate/submit_generation/open_asset 的选用时机)。生产实测 2/2:「我想把一个中文视频翻译成英文配音」→ 助手立即 navigate_view(dub) + 四步流程解答;「训练我自己画风的模型」→ search_knowledge 命中 + list_models 查证底模。embedding 服务在线,索引按内容 hash 首用重建。pytest test_rag_cache 3 passed。

- 2026-09-02 应用运行页双模式(简洁/工作流,commits 15c0d47+022a5af,已部署+生产实测):用户要求市场应用打开分简洁/工作流两种显示,工作流展现给用户最可控。①后端——GET /api/apps/{id} 的 workflow_json 对所有可见用户透出(原仅属主/admin),可见性/NSFW 门控不变,test_apps 契约改写。②前端——lib/apps 增 AppWorkflowNode/AppBinding 类型+归一(非法剔除)、orderWorkflowNodes(Kahn 拓扑,环回退原序)、bindingsByNode 分组;新组件 AppWorkflowGraph:节点拓扑纵向成流、绑定参数在所属节点内联编辑(与简洁模式共享 values)、只读叶子标量直读+连线 ← #id、绑定节点 accent 左缘+「可调」徽标、节点间下箭头连接;AppRunnerView 页头「简洁|工作流」段控,提交/进度/结果区双模式共享。③简洁模式美化:表单升抬底面板卡(白底+细边+xs 投影+行距放宽)。④生产实测抓获修复:长工作流下段控滚出视口 → 运行页页头 sticky。生产实测:Flux2 文生图 11 节点全渲染、跨模式值共享正常、控制台零 JS 错误。回归:pytest 2983 / npm test 937(+8)/ tsc 0 / 干净 build。

- 2026-09-02 组件级美学精修(commits 249b92b+2a124fe,已部署 core+生产实测):用户反馈「全站输入框太丑,组件整体优化」。根因:.input 灰填充(surface-3)+无边框=沉重廉价、原生 select 无箭头、按钮无层次。①.input 弃灰填充→抬底 surface-1 + hairline 描边 + 新 token --shadow-xs(whisper 投影);hover 描边提强;focus 墨线 accent + 3px accent-glow 柔光环;caret 跟 accent;新增 disabled/aria-invalid 错误态(红边红环)。②select.input appearance:none + Lucide chevron-down data-URI(亮 #64666C/暗 #9A9EA6 双色)。③.btn 抬底+xs 投影、hover surface-2+sm;primary 叠 md→hover lg。④.at-seg 激活墨丸补 sm 投影真正浮起。⑤主页对话框 av-composer-box 补 xs 投影 + focus 光环。生产实测(浏览器+新签 admin token,旧 .auth token 已过期):登录页/首页 focus 光环/作品库搜索框/参数面板全命中;**抓获真 bug**:.generate-params .input 覆盖块 background 简写把 select chevron 的 background-image 重置为 none——该块与全局同构整块删除,生产 CSS 复验 bad-override=0。另补回上轮 IDE 回滚吞掉的 stage.css 两处(promptbar-error 删 + 骨架行样式)。回归:npm test 929 / tsc 0 / 干净 build。遗留观察:作品库视频 URL query 带 JWT(进历史/日志),建议短时效签名化(未动)。

- 2026-09-01 W4 死代码清理 + W2 勘察落地(commits 1a74243+7804c2a,已部署 core):①死代码——删 lib/utils.ts / lib/workflows.ts / usePlaybackInsights.ts(连带 api.ts getDramaPlaybackInsights 及类型,后端端点保留);cornernav.css 删 .cornernav* 207 行(现存 AccountButton+TaskCenter 两节),删 cornernav 诊断 e2e+截图目录;globals.css 删零消费 --text-display-lg。②W2 勘察(subagent 全仓证据)——三页工作台页头/空态/masthead 已达标;落地修复:stage.css 死样式删(nsfw-workbench/nsfw-library/generate-header/generate-header-note/promptbar-error)、补缺 .stage-skeleton-lines/.stage-skeleton-line(生成中骨架行此前无样式=真 bug)、GenerateView 死 import 删、symmetry-check e2e 死选择器更新。③勘察遗留(下轮候选,涉设计决策未自动做):音频页编辑 tab 卡片表单范式 vs 舞台范式分裂;参数面板说明文字残留;面板开合不持久化/不可调宽。回归:npm test 929 全绿 / tsc 0 / 干净 build;生产 CSS 实证 cornernav-trigger=0。注意:github 推送网络超时未成功,origin(gitee)已最新,github 待补推 1a74243+7804c2a。

- 2026-09-01 部署:masthead 清理 + 本机缓存三层扩展合入 `4baeea7`,双推 gitee+github,deploy.sh → core(toiv-api/toiv-web 健康 200,回滚快照 .rollback-previous 在位);生产 CSS 实证 `page-header-kicker` 计数 0、`empty-console-hint` 在位。

- 2026-09-01 本机缓存三层扩展 L1+L2+L3(本地验证,未 commit 未部署):用户目标「增强流畅性,缓存保留在用户电脑」,定稿分层方案(否掉「全部进 localStorage」:5MB 配额+实时数据不宜缓存)。①L0 基建——swr-cache.ts 升三层:内存 Map → localStorage(小负载)→ **IndexedDB**(`opts.large`,大负载 50MB+ 配额、异步不卡主线程、不可用静默降级仅内存);新增 `prime()` 回种(强制刷新流程用);invalidate/invalidatePrefix 同步清 IDB。②L1 扩覆盖——engines(60s,/nsfw 分轨防 R18 污染,「重新检测」prime 回种+作废另一档)、会话列表(30s,large→IDB)、主体库(5min,手写模块缓存替换为 swr)、应用市场(5min,过滤后缀分键)、agent-runs(15s)、studio 项目(30s)全部接入;写路径显式失效:会话删除/分叉/对话完成(agentChat+stream+resume 三处)、实体 CRUD、apps fork/run/confirmImport、agent-runs create/cancel、studio create/patch/delete。③L3 预取——新建 lib/prefetch.ts 视图→首屏数据预热映射;SideRail 悬停/聚焦在原有 chunk 预热外挂数据预取(prefetchView);登录落地后 requestIdleCallback 空闲预热 image/video/library/market。④测试——新增 cacheExtend.test.ts 11 例(prime 命中/失效重取/in-flight 去重/stale 后台刷新/接线断言/IDB 降级);appsApi beforeEach 加 invalidatePrefix 防用例间缓存污染;assistantPopupSessions 断言随 listAgentSessions 改非 async 导出更新。过程:IDE 缓冲区回滚 6+ 次(api.ts 四段反复、appsApi/page.tsx import、swr-cache prime 段),逐项 grep 复核补回,并修复回滚造成的 getAgentSession 误植 fork 代码错乱。回归:npm test 929 全绿(+11)/ tsc 0 / 干净 build;后端零改动未跑 pytest。

- 2026-09-01 masthead 退役·全站 kicker/大标题清理(本地验证,未 commit 未部署):用户指令「把其他页面中类似这样效果的文字和样式也删掉」(旧生成页空态 PROMPT ATELIER 式版型)。①kicker 铭牌清零——13 视图 PageHeader `kicker=` prop 全删(Admin/Animatic/Backlot/AvatarTalk/Studio/Resources/Train/Entities/Market/Settings/Dub/Observability/Models);agent-runs 手写 `page-header-kicker` span 删除;library.css `content:"MEDIA ATELIER"` ::before 删除;AssistantView 弹窗空态 `av-empty-kicker`「AI ASSISTANT」删除。②masthead 版型退役——globals.css v6.1 块重写:.page-header-kicker 样式删、标题 Fraunces display-md→无衬线 --text-title/semibold(基础块补 display:flex 图标对齐 + actions flex-shrink:0)、编辑双线 ::after 删改单根 subtle hairline;library.css 页头覆盖整段删除走全局(含移动端重复降档);at-placeholder-title/av-empty-title 去 Fraunces 收敛标题档。③死代码——stage.css `.generate-results:has(.empty-editorial)` 空态例外删(空态已单行化不再溢出 72vh);ResourcesView 内嵌页头 kicker 隐藏覆盖删。④测试——uiPrimitives kicker 用例改写为退役断言+onBack 分支;uiPolishViews ①改写为英雄区退役断言(.empty-console-hint muted/aux);generateInspector T3 QuickStartGrid 四用例删除(组件已于上轮删除)改退役源码断言、⑩类名表去 quick-start 加 empty-console-hint;mobileLayout 页头标题降档断言移除(全局已收敛);observability OBSERVABILITY 断言改 doesNotMatch;symmetry-check e2e `.empty-editorial`→`.result-panel-empty`。过程:IDE 缓冲区回滚 3 次复发(generateInspector 两段、agent-runs+AssistantView、stage.css),逐项 grep 复核补回。回归:npm test 918 全绿 / tsc 0 / 干净 build;后端零改动未跑 pytest。

- 2026-08-31 Studio Console v1 极简专业工作台骨架(用户定稿方案,已部署+生产实测):①新骨架——左侧 52px 常驻图标栏 SideRail(8 高频项+admin 分隔区+底部任务中心/账户槽,grid 流内成员不与绝对定位视图叠层),CornerNav/灵动岛/顶部账户钮/任务中心全部退役;⌘K 命令面板(动作/页面/会话/作品四源检索,↑↓/Enter/Esc,新对话与开会话经 CustomEvent+PENDING_SESSION_KEY 跨视图暂存);Popover 面板自动向上翻转(锚点在视口下半部时)。②首页对话极简——空态只剩 TOIV 铭牌+无边框输入框+模型行(引擎胶囊/场景 chip/快捷 chip/最近作品/微粒场/点阵/页头全退役,ParticleField 组件删除);消息流文档式(无头像无气泡,助手全文宽,用户右对齐灰字);历史/新对话收进输入框工具行;模型设置面板退役。③主题系统 v8 单色中性系——v7 五色板(wood/mono/mint/apricot)+自定义 accent 全部退役,:root+dark 双模式+pureBlack 子档,lib/theme.ts 重写(旧 key 读取时迁移清除),ThemePicker 只剩明暗段控+纯黑开关,layout.tsx 防 FOUC 脚本同步精简。生产实测 7/7:空态居中/左栏指示条+tooltip/⌘K 过滤跳转/新对话回 home/文档式消息流无英文推理/底部弹层向上展开不裁切/控制台零真实 JS 错误;复核抓获两小修(指示条 -8→-6px 不贴屏缘、空态 min-height:100% 居中生效)已部署复验。回归:pytest 2983 / npm test 923(+5 新骨架,+全量旧契约改写)/ tsc 0 / 干净 build。

- 2026-08-31 前端改造 W4+W5 视觉焕新·紧凑化·降级路径(已部署 core+生产实测):①紧凑化(用户反馈「文字太多」)——对话首页页头砍标题/描述(§10 CornerNav 豁免),历史/新建/设置纯图标化;场景入口 5 大卡片→单行 chip(短剧或视频/图像/音频/数字人/译制,双模式各 5 枚对称,去「作品库」与最近作品区去重);快捷提示词 2×2 大卡(含 36 字 prompt 预览)→单行 ghost chip(完整 prompt 收 title);MODEL_DESC 陈旧文案(L1)连同设置面板「模型说明」组退役;模型徽标改读真实 display_model(core .env 修正为 Qwen3.8-Flash-Next (Spark×2 · 262K),备份 .env.bak-20260831-displayname)。②微粒场(ParticleField,新组件):Canvas 2D,桌面 160/窄屏 80(硬上限 300),速度上限 0.45px/帧(红线 1.2),z 景深三档软斑,6s 呼吸,指针缓随+点击水波(慢速大范围)+chip 悬停汇聚锚点,暗角层,双色(accent+muted)主题联动 MutationObserver,reduced-motion 静态一帧,标签页隐藏暂停,pointer-events none 不挡内容。③产物胶片条:消息媒体/作业卡 done 产物 ≥2 视觉帧合并横向胶片条(导轨打孔+帧号 01…,scroll-snap),单帧保持 hero;同消息 ≥2 done 作业卡产物聚合一条(aggregateJobFrames)。④推理碎片抑制(runner):带 tool_calls 轮次的伴生文本(英文自述)不下发不落库,working copy 保留维持模型上下文。⑤生产实测抓获修复:破图(renderAvMedia/renderAvFrames 渲染时经 imageUrl 补 token——/api/images 强制登录态,裸 sig URL 在 <img> 下 401);页形态三面板 Esc 统一关闭+桌面端面板顶让位 56px chrome 带(此前右面板关闭钮与账户头像重叠被拦截);回放链路双修复——runner 落库 job done 事件 data.results 为 tool 媒体(此前只认顶层 urls,异步产物回放全丢)+ GET /agent/sessions/{sid} 按 submit_generation 工具消息 job_id 回填 Job 表最新产物(对话结束后完成的作业回放可见,仅 done/限本人/空 media 才补),前端 messagesToChat 跳过空工具轮空气泡+按 URL 去重。生产实测:空态紧凑微粒首页、4 图聚合胶片条(01-04 帧号)、历史会话回放 2 猫图胶片条无破图无空气泡、navigate_view 音频页跳转、控制台零真实 JS 错误。回归:pytest 2983(+3:推理抑制/job 媒体落库/回填端点)/ npm test 934(+15 W4W5)/ tsc 0 / 干净 build。

- 2026-08-31 前端改造 W3 助手 UI 驱动(c93cc58,已部署+生产实测):三工具 navigate_view/prefill_generate/open_asset 注册进 tool_seam;前端 ui_action 事件即收即执行;prefill 经 toiv_engine_draft 草稿回填工作台。生产实测「带我去译制页面」「打开作品库」3 秒内跳转生效(早前失效系旧标签页缓存,硬刷新后恢复)。回归:pytest 49(W3 相关)/ npm test 919。
