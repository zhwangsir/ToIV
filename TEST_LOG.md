...
- 2026-09-10 admin soft-hide 可再上架 + v20 P0/animate2 unhide 候选（项目管家；ToIV 开发+能力缺口 tip；docs；产品 dirty 待用户提交）：产品未提交 — `apps.py` admin `_visible` 可见 soft-hide 并可 PUT 再上架（seed 不覆盖 is_public）；`test_admin_can_unhide_soft_hidden_catalog_and_builtin`；原因=mass soft-hide 后 admin 404。**待用户产品 commit**（apps.py/tests 仍 dirty 勿 stage）。能力缺口：通用 P0 **80** + `ok_if_dedicated_animate2` **2**（`wan-animate-2` + `rh-character-replacement-minimax-h3-7582cc8e` → `:8199`）；可先 unhide 该 2 → 公开约 **508→510**；真源 `.regen_tmp/.../unhide_after_fill_p0.json` **勿 stage**；产品 unhide 仍等开发落地或用户拍板。勿 stage `.regen_tmp`/产品 dirty；无产品 SHA；status=`pending_product_commit`；STATE `admin_soft_hide_relist_wan_animate2_2026_09_10`；updated_at 2026-09-10T15:40:00+08:00；AGENTS §七 已短注。
